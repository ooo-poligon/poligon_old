...
// *****************************************************************************************
	$ForumsPerms = array(
		"Q"=>GetMessage("LU_USER_Q"), 
		"U"=>GetMessage("LU_USER_U"), 
		"Y"=>GetMessage("LU_USER_Y"), 
		"user"=>GetMessage("LU_USER_USER"));
	$strError = "";
// *****************************************************************************************
...
// ******Filter*****************************************************************************
	...
	if ((strLen($_REQUEST["del_filter"]) <= 0) && (strLen($_REQUEST["set_filter"]) > 0))
	{
		...
		$arResult["filter"]["~user_name"] = trim($_REQUEST["user_name"]);
		$arResult["filter"]["user_name"] = htmlspecialchars(trim($_REQUEST["user_name"]));
		if (!empty($arResult["filter"]["~user_name"]))
			$arFilter["SHOW_ABC"] = $arResult["filter"]["~user_name"];
	}
	else 
	{
	...
	if($db_res)
	{
		...
		if ($res = $db_res->GetNext())
		{
			do
			{
				...
				$res["user_post"] =  CComponentEngine::MakePathFromTemplate($arParams["URL_TEMPLATES_USER_POST"], array("UID" => $res["USER_ID"], "mode" => "all"));
