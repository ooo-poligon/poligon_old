					...
					else
					{
						...
						foreach ($arPropertyValue as $key => $value)
						{
							...
							if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["MULTIPLE"] == "Y")
								...
								$arUpdatePropertyValues[$propertyID][$arParams["ID"].":".$propertyID] = $arFile;

							if ($arParams["MAX_FILE_SIZE"] > 0 && $arFile["size"] > $arParams["MAX_FILE_SIZE"]) $arResult["ERRORS"][] = GetMessage("IBLOCK_ERROR_FILE_TOO_LARGE");
						}
