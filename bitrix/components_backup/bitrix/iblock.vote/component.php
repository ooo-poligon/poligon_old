	...
	if($RATING>0 && $RATING<=$arParams["MAX_VOTE"])
	{
		...
		if($ELEMENT_ID>0 && !array_key_exists($ELEMENT_ID, $_SESSION["IBLOCK_RATING"]))
		{
			...
			if($res)
			{
				...
				CIBlockElement::SetPropertyValues($ELEMENT_ID, $arParams["IBLOCK_ID"], $arProperties["vote_count"]["VALUE"], "vote_count");
				CIBlockElement::SetPropertyValues($ELEMENT_ID, $arParams["IBLOCK_ID"], $arProperties["vote_sum"]["VALUE"], "vote_sum");
				CIBlockElement::SetPropertyValues($ELEMENT_ID, $arParams["IBLOCK_ID"], $arProperties["rating"]["VALUE"], "rating");
