...
// *****************************************************************************************
	if (in_array("USERS_ONLINE", $arParams["SHOW"]))
	{
		...
		$arFields = array(
			">=LAST_VISIT" => Date(CDatabase::DateFormatToPHP(CLang::GetDateFormat("FULL", LANG)), time()-$arParams["PERIOD"]), 
			"COUNT_GUEST"=>true);
		...
		$cache_path = "/".SITE_ID."/forum/statistic/user_online/";
...
// *****************************************************************************************
	if (in_array("BIRTHDAY", $arParams["SHOW"]))
	{
		...
		$cache_path = "/".SITE_ID."/forum/statistic/birthday/";
...
// *****************************************************************************************
	if (in_array("STATISTIC", $arParams["SHOW"]))
	{
		$cache_id = serialize(array(
			...
			$arParams["FORUM_ID"],
			$arParams["FID"]));
		$cache_path = "/".SITE_ID."/forum/statistic/forums/";
		if ($arParams["CACHE_TIME"] > 0 && $cache->InitCache($arParams["CACHE_TIME"], $cache_id, $cache_path))
		{
			$res = $cache->GetVars();
