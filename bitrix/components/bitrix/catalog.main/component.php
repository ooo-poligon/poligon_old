...
if($this->StartResultCache(false, $USER->GetGroups()))
{
	...
	while($arIBlock = $rsIBlocks->GetNext())
	{
		...
		$arIBlock["LIST_PAGE_URL"] = htmlspecialchars(str_replace(
			array("#SERVER_NAME#", "#SITE_DIR#", "#IBLOCK_ID#"),
			array(SITE_SERVER_NAME, SITE_DIR, $arIBlock["ID"]),
			strlen($arParams["IBLOCK_URL"])? $arParams["IBLOCK_URL"]: $arIBlock["LIST_PAGE_URL"]
		));
		$arIBlock["LIST_PAGE_URL"] = str_replace("//", "/", $arIBlock["LIST_PAGE_URL"]);
