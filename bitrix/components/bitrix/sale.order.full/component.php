...
if (!$USER->IsAuthorized())
{
	...
	elseif ($arResult["POST"]["do_register"] == "Y" && $arResult["AUTH"]["new_user_registration"] == "Y")
	{
		...
		if ($arResult["POST"]["NEW_GENERATE"] == "Y")
		{
			...
			$arResult["POST"]["~NEW_PASSWORD"] = GetRandomCode(6);
			$arResult["POST"]["~NEW_PASSWORD_CONFIRM"] = $arResult["POST"]["~NEW_PASSWORD"];
		}
		else
		{
		...
		if (strlen($arResult["ERROR_MESSAGE"]) <= 0)
		{
			...
			else
			{
				if ($USER->IsAuthorized())
				{
					CUser::SendUserInfo($USER->GetID(), SITE_ID, GetMessage("INFO_REQ"));
	...
	if ($arResult["CurrentStep"] > 0 && $arResult["CurrentStep"] <= 6)
	{
		...
		while ($arBasketItems = $dbBasketItems->GetNext())
		{
			...
			if ($arBasketItems["DELAY"] == "N" && $arBasketItems["CAN_BUY"] == "Y")
			{
				...
				$arResult["ORDER_WEIGHT"] += DoubleVal($arBasketItems["WEIGHT"]) * DoubleVal($arBasketItems["QUANTITY"]);
				...
				if(DoubleVal($arBasketItems["VAT_RATE"]) > 0)
				{
					...
					$arBasketItems["PRICE_VAT_VALUE"] = (($arBasketItems["PRICE"] / ($arBasketItems["VAT_RATE"] +1)) * $arBasketItems["VAT_RATE"]);
					
					$arResult["vatSum"] += DoubleVal($arBasketItems["PRICE_VAT_VALUE"]) * DoubleVal($arBasketItems["QUANTITY"]);
				}
			}
		}
		...
		if (strlen($arResult["ERROR_MESSAGE"]) <= 0 && $arResult["CurrentStep"] > 3)
		{
			...
			if (is_array($arResult["DELIVERY_ID"]))
			{
				...
				if ($arDeliveryPrice["RESULT"] == "ERROR")
					...
					$arResult["DELIVERY_PRICE"] = roundEx($arDeliveryPrice["VALUE"], SALE_VALUE_PRESICION);
			}
			else
			{
			...
			if ($arDiscount = $dbDiscount->Fetch())
			{
				if ($arDiscount["DISCOUNT_TYPE"] == "P")
				{
					...
					for ($bi = 0; $bi < count($arProductsInBasket); $bi++)
					{
						...
						{
							$curDiscount = roundEx(DoubleVal($arProductsInBasket[$bi]["PRICE"]) * IntVal($arProductsInBasket[$bi]["QUANTITY"]) * $arDiscount["DISCOUNT_VALUE"] / 100, SALE_VALUE_PRECISION);
							$arDiscounts[IntVal($arProductsInBasket[$bi]["ID"])] = roundEx($curDiscount / IntVal($arProductsInBasket[$bi]["QUANTITY"]), IntVal($arProductsInBasket[$bi]["QUANTITY"]));
						...
						else
						{
							...
							$arResult["DISCOUNT_PRICE"] += $curDiscount * IntVal($arProductsInBasket[$bi]["QUANTITY"]);
				...
				else
				{
					...
					for ($bi = 0; $bi < count($arProductsInBasket); $bi++)
					{
						$curDiscount = roundEx(DoubleVal($arProductsInBasket[$bi]["PRICE"]) * $arResult["DISCOUNT_PRICE"] / $arResult["ORDER_PRICE"], SALE_VALUE_PRECISION);
						$arDiscounts[IntVal($arProductsInBasket[$bi]["ID"])] = $curDiscount;
						$arProductsInBasket[$bi]["DISCOUNT_PRICE"] = DoubleVal($arProductsInBasket[$bi]["PRICE"]) - $curDiscount;
						$arResult["DISCOUNT_PRICE_tmp"] += $curDiscount * IntVal($arProductsInBasket[$bi]["QUANTITY"]);
					}
	...
	if ($arResult["CurrentStep"] == 3)
	{
		if (IntVal($arResult["DELIVERY_LOCATION"]) > 0)
		{
			...
			foreach ($arDeliveryServicesList as $key => $arDelivery)
			{
				...
				if ($cnt <= 0) unset($arDeliveryServicesList[$key]);
				else 
				{
					$numDelivery += $cnt;
				}
		...
		while ($arBasketItems = $dbBasketItems->Fetch())
		{
			...
			if ($arBasketItems["DELAY"] == "N" && $arBasketItems["CAN_BUY"] == "Y")
			{
				...
				if(DoubleVal($arBasketItems["DISCOUNT_PRICE"]) > 0)
				{
					...
					$arBasketItems["DISCOUNT_PRICE_PERCENT"] = $arBasketItems["DISCOUNT_PRICE"]*100 / ($arBasketItems["DISCOUNT_PRICE"] + $arBasketItems["PRICE"] - DoubleVal($arBasketItems["VAT_VALUE"]));
	...
	elseif ($arResult["CurrentStep"] == 7)
	{
		...
		if ($arOrder = $dbOrder->GetNext())
		{
			...
			if (is_array($arDateInsert) && count($arDateInsert) > 0)
				$arOrder["DATE_INSERT_FORMATED"] = $arDateInsert[0];
			else
				$arOrder["DATE_INSERT_FORMATED"] = $arOrder["DATE_INSERT"];
