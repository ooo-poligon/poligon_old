...
if (!$bHasPath)
{
	...
	if ($arParams["AREA_FILE_SHOW"] == "page")
	{
		...
		else
		{
			...
			$sFileName = substr($APPLICATION->GetCurPage(), 0, strlen($APPLICATION->GetCurPage())-4)."_".$arParams["AREA_FILE_SUFFIX"].".php";
		...
		if ($sPerm1 >= 'W')
		{
			...
			if ($showMode == 'configure')
			{
				$arIcons[] = array(
					"URL" => 'javascript:'.$APPLICATION->GetPopupLink(
						array(
							'URL' => "/bitrix/admin/public_file_edit_src.php?lang=".LANGUAGE_ID."&from=main.include&template=".urlencode($arParams["EDIT_TEMPLATE"])."&path=".urlencode($sFilePath.$sFileName).$editor,
							"PARAMS" => array(
								'width' => 770,
								'height' => 570,
								'resize' => true
		...
		if ($sFilePath != $sFilePathTMP && $sPerm2 >= 'W')
		{
			...
			if ($showMode == 'configure')
			{
				$arIcons[] = array(
					"URL" => 'javascript:'.$APPLICATION->GetPopupLink(
							array(
								'URL' => "/bitrix/admin/public_file_edit_src.php?lang=".LANGUAGE_ID."&from=main.include&new=Y&path=".urlencode($sFilePathTMP.$sFileName)."&new=Y&template=".urlencode($arParams["EDIT_TEMPLATE"]).$editor,
								"PARAMS" => array(
									'width' => 770,
									'height' => 570,
									'resize' => true
	...
	elseif ($sPerm2 >= 'W')
	{
		...
		if ($showMode == 'configure')
		{
			$arIcons[] = array(
				"URL" => 'javascript:'.$APPLICATION->GetPopupLink(
					array(
						'URL' => "/bitrix/admin/public_file_edit_src.php?lang=".LANGUAGE_ID."&from=main.include&path=".urlencode($sFilePathTMP.$sFileName)."&new=Y&template=".urlencode($arParams["EDIT_TEMPLATE"]).$editor,
						"PARAMS" => array(
							'width' => 770,
							'height' => 570,
							'resize' => true
...
if ($bFileFound) include($_SERVER["DOCUMENT_ROOT"].$sFilePath.$sFileName);
?>