...
if ($REQUEST_METHOD=="GET" && strlen($RestoreDefaults)>0 && $SALE_RIGHT=="W")
{
	$bWasUpdated = true;
...
$strWarning = "";
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $SALE_RIGHT=="W")
{
	$bWasUpdated = true;
	...
	{
		if(DoubleVal($val) > 0)
			$arAmountSer[$key] = Array("AMOUNT" => $val, "CURRENCY" => $amount_currency[$key]);
	}
	...
	while ($arPersonType = $dbPersonType->GetNext())
	{
		...
		{
			$arActFields = split(",", ${"export_fields_".$arPersonType["ID"]});
...
$tabControl->BeginNextTab();
	...
	<tr class="heading">
		<td colspan="2">Суммы для зачисления на счет</td>
	</tr>
	<tr>
...
var cur_site = {WEIGHT:'<?=htmlspecialchars($siteList[0]["ID"])?>',ADDRESS:'<?=htmlspecialchars($siteList[0]["ID"])?>'};
...
function setWeightValue(obj)
{
	...
	<tr>
		...
		<td valign="top"><select name="site" id="WEIGHT_site_id"<? if(COption::GetOptionString($module_id, "WEIGHT_different_set", "N") != "Y") echo " disabled=\"disabled\""; ?> OnChange="selectSite(this.value, 'WEIGHT')">
			...
			?></select><input type="hidden" name="WEIGHT_current_site" id="WEIGHT_current_site" value="<?=htmlspecialchars($siteList[0]["ID"]);?>" /></td>
	</tr>
	<tr>
		<td valign="top" colspan="2">
	<?for ($i = 0; $i < $siteCount; $i++):?>
			<div id="par_WEIGHT_<?=htmlspecialchars($siteList[$i]["ID"])?>" style="display: <?=($i == 0 ? "inline" : "none");?>">
	...
	<tr>
		...
		<td valign="top"><select name="site" id="ADDRESS_site_id"<? if(COption::GetOptionString($module_id, "ADDRESS_different_set", "N") != "Y") echo " disabled=\"disabled\""; ?> onChange="selectSite(this.value, 'ADDRESS')">
			<?
				for($i = 0; $i < $siteCount; $i++)
					echo "<option value=\"".htmlspecialchars($siteList[$i]["ID"])."\">".htmlspecialchars($siteList[$i]["NAME"])."</option>";
			?></select><input type="hidden" name="ADDRESS_current_site" id="ADDRESS_current_site" value="<?=htmlspecialchars($siteList[0]["ID"]);?>" /></td>
	</tr>
	<tr>
...
?>
		<div  id="par_ADDRESS_<?=htmlspecialchars($siteList[$i]["ID"])?>" style="display: <?=($i == 0 ? "inline" : "none");?>">
		<table cellpadding="0" cellspacing="2" border="0" width="60%" align="center">
			<tr class="heading">
...
<?$tabControl->BeginNextTab();?>
	...
	<tr>
		...
		while ($db_result_lang_array = $db_result_lang->Fetch())
		{
			...
			<tr>
				<td valign="top">
					[<a href="site_edit.php?LID=<?=$db_result_lang_array["LID"]?>&lang=<?=LANGUAGE_ID?>" title="<?=GetMessage("SALE_SITE_ALT")?>"><?echo $db_result_lang_array["LID"] ?></a>] <?echo htmlspecialchars($db_result_lang_array["NAME"]) ?>
				</td>
				<td valign="top">

					<?
					$arCurr = CSaleLang::GetByID($db_result_lang_array["LID"]);
					echo CCurrency::SelectBox("CURRENCY_".$db_result_lang_array["LID"], $arCurr["CURRENCY"], GetMessage("SALE_NOT_SET"), True, "");
					?>
				...
				<td valign="top">
					...
					$dbSiteGroupsList = CSaleGroupAccessToSite::GetList(
							array(),
							array("SITE_ID" => $db_result_lang_array["LID"])
						);
					...
					?>
					<select name="SITE_USER_GROUPS_<?= $db_result_lang_array["LID"] ?>[]" multiple size="5">
						<?
