...
<body bgColor="#ffffff">
<p><b>ПОСТАВЩИК:</b>
<?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("SELLER_NAME")) ?><br>
Адрес: <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("SELLER_ADDRESS")) ?><br>
Телефон: <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("SELLER_PHONE")) ?><br>
ИНН: <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("SELLER_INN")) ?> / КПП: <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("SELLER_KPP")) ?><br>
Банковские реквизиты:<br>
<?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("SELLER_RS")) ?><br>
к/с <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("SELLER_KS")) ?><br>
БИК <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("SELLER_BIK")) ?>
</p>

<p><b>ЗАКАЗЧИК:</b>
<?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("BUYER_NAME")) ?><br>
ИНН: <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("BUYER_INN")) ?><br>
Адрес: <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("BUYER_ADDRESS")) ?><br>
Телефон: <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("BUYER_PHONE")) ?><br>
Факс: <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("BUYER_FAX")) ?><br>
Контактное лицо: <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("BUYER_PAYER_NAME")) ?><br>
...
<p><b>СЧЕТ N:</b> <?= $ORDER_ID ?> от  <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("DATE_INSERT")) ?></p>
...
if ($arBasket = $dbBasket->Fetch()):
	...
	do
	{
		...
		<tr valign="top">
			...
			<td bgcolor="#ffffff" style="border: 1pt solid #000000; border-right:none; border-top:none;">
				<?= htmlspecialcharsEx("[".$arBasket["ID"]."] ".$arBasket["NAME"]); ?>
			</td>
			<td align="right" bgcolor="#ffffff" style="border: 1pt solid #000000; border-right:none; border-top:none;">
				<?= $arBasket["QUANTITY"]; ?>
			</td>
			<td align="right" bgcolor="#ffffff" style="border: 1pt solid #000000; border-right:none; border-top:none;">
				<?= SaleFormatCurrency($arBasket["PRICE"]-$arBasket["DISCOUNT_PRICE"], $arBasket["CURRENCY"], true) ?>
			</td>
			<td align="right" bgcolor="#ffffff" style="border: 1pt solid #000000; border-top:none;">
				<?= SaleFormatCurrency(($arBasket["PRICE"]-$arBasket["DISCOUNT_PRICE"])*$arBasket["QUANTITY"], $arBasket["CURRENCY"], true) ?>
			</td>
		</tr>
		<?
		$sum += doubleval(($arBasket["PRICE"]-$arBasket["DISCOUNT_PRICE"])*$arBasket["QUANTITY"]);
	}
	...
	<tr>
		...
		<td bgcolor="#ffffff" style="border: 1pt solid #000000; border-right:none; border-top:none;">
			...
			echo htmlspecialcharsEx($arDelivery_tmp["NAME"]);
...
<p><b>Итого к оплате:</b> 
	...
	?></p>
<p>
<div align="right" style="padding-right:30px;"><?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("DATE_INSERT")) ?></div><br />
<?
...
В платежном поручении обязательно указать - "Оплата по счету № <?echo $ORDER_ID ?> от <?= htmlspecialcharsEx(CSalePaySystemAction::GetParamValue("DATE_INSERT")) ?>".<br><br>
