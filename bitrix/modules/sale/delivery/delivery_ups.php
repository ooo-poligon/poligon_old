...
class CDeliveryUPS
{
	...
	function __GetLocation(&$arLocation, $arConfig)
	{
		...
		{
			if (
				stristr($arZone[0], $arLocation["COUNTRY_NAME_ORIG"]) !== false
				|| stristr($arZone[0], $arLocation["COUNTRY_SHORT_NAME"]) !== false
				|| stristr($arZone[0], $arLocation["COUNTRY_NAME_LANG"]) !== false
				|| stristr($arLocation["COUNTRY_NAME_ORIG"], $arZone[0]) !== false
				|| stristr($arLocation["COUNTRY_SHORT_NAME"], $arZone[0]) !== false
				|| stristr($arLocation["COUNTRY_NAME_LANG"], $arZone[0]) !== false
			)
			{
	...
	function Calculate($profile, $arConfig, $arOrder, $STEP, $TEMP = false)
	{
		...
		while (doubleval($arOrder["WEIGHT"]) > doubleval($key));
	...
	{
		if (intval($arOrder["LOCATION_FROM"]) <= 0) return array();

		$arLocationFrom = CSaleLocation::GetByID($arOrder["LOCATION_FROM"]);
		$arLocationTo = CSaleLocation::GetByID($arOrder["LOCATION_TO"]);

		if ($arLocationFrom["COUNTRY_ID"] == $arLocationTo["COUNTRY_ID"]) return array();

		CDeliveryUPS::__GetLocation($arLocationTo, $arConfig);
		if (strlen($arLocationTo["COUNTRY_SID"]) <= 0) return array();
		...
		if (intval($arZoneTo[1]) > 0) return array("express", "express_saver");
		else return array("express");
	}
}
