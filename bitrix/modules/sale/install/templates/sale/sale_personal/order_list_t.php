		...
		<td valign="top" width="60%">
			...
			while ($arStatus = $dbStatus->Fetch())
			{
				...
				if ($arOrderList = $dbOrderList->Fetch())
				{
					$bNoOrders = False;
					?>
					<font class="tabletitletext"><b><?echo GetMessage("STPOL_ORDER_STATUS")?> "<?= $arStatus["NAME"] ?>"</b></font><br>
						...
						<table border="0" cellspacing="0" cellpadding="3" width="100%">
							<tr>
								<td class="tablebody">
									<font class="tableheadtext">
										...
										<a title="<?echo GetMessage("STPOL_DETAIL_LINK_ALT")?>" href="<?= htmlspecialchars($PATH_TO_DETAIL) ?>?ID=<?= $arOrderList["ID"] ?>&SHOW_TYPE=<?= $showType ?>"><?= $arOrderList["ID"] ?></a>
							...
							<tr>
								<td class="tablebody" align="right">
									<font class="tablebodytext">
									<a title="<?= GetMessage("STPOL_DETAIL_LINK_ALT") ?>" href="<?= htmlspecialchars($PATH_TO_DETAIL) ?>?ID=<?= $arOrderList["ID"] ?>&SHOW_TYPE=<?= $showType ?>"><?= GetMessage("STPOL_DETAILS") ?></a>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
