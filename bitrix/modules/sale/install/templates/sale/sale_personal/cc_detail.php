...
if ($bVarsFromForm)
{
	...
	for ($i = 0; $i < count($arCardFields); $i++)
		if (array_key_exists($arCardFields[$i], $_REQUEST))
			${"str_".$arCardFields[$i]} = htmlspecialchars($_REQUEST[$arCardFields[$i]]);
}
?>
...
<a name="tb"></a>
<font class="text">
<a href="<?= htmlspecialchars($PATH_TO_LIST) ?>"><?= GetMessage("STPC_TO_LIST") ?></a>
</font>
<br><br>
