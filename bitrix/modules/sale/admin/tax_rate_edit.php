...
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $saleModulePermissions=="W" && check_bitrix_sessid())
{
	...
	// налоги только в процентах. Иначе надо менять CSaleOrderTax::CountTaxes
	$IS_PERCENT = "Y";
...
$sDocTitle = ($ID>0) ? eregi_replace("#ID#", "$ID", GetMessage("TAX_RATE_EDIT_RECORD")) : GetMessage("TAX_RATE_NEW_RECORD");
...
$aMenu = array(
		array(
				...
				"ICON" => "btn_list",
				"LINK" => "/bitrix/admin/sale_tax_rate.php?lang=".LANG."&".GetFilterParams("filter_", false)
			)
	);
...
if ($ID > 0 && $saleModulePermissions >= "W")
{
	...
	$aMenu[] = array(
			...
			"ICON" => "btn_new",
			"LINK" => "/bitrix/admin/sale_tax_rate_edit.php?lang=".LANG."&".GetFilterParams("filter_", false)
		);
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/bitrix/admin/sale_tax_rate.php?lang=".LANG."&".GetFilterParams("filter_", false)
			)
	);
?>
