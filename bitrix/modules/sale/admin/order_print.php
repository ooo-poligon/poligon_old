...
$APPLICATION->SetTitle(eregi_replace("#ID#", "$ID", GetMessage("SALE_PRINT_RECORD")));
...
if ($REQUEST_METHOD == "POST" && strlen($Print)>0 && check_bitrix_sessid() && $bUserCanViewOrder)
{
	if(count($REPORT_ID) > 0)
	{
		...
		{
			?>
			window.open('sale_print.php?doc=<?echo $REPORT_ID[$i] ?>&ORDER_ID=<?echo $ID ?>&BASKET_IDS=<?echo urlencode($sBasket) ?>&QUANTITIES=<?echo urlencode($sQuantity) ?>', '_blank');
			<?
		}
		?>
		</script>
...
$aMenu = array(
		array(
				...
				"LINK" => "/bitrix/admin/sale_order.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false)
			)
	);
