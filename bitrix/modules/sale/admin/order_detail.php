	...
	&& empty($dontsave))
{
	...
	if (strlen($errorMessage) <= 0)
	{
		if (!CSaleOrder::DeliverOrder($ID, $ALLOW_DELIVERY))
		{
			if ($ex = $APPLICATION->GetException())
				$errorMessage .= $ex->GetString();
			else
...
$APPLICATION->SetTitle(eregi_replace("#ID#", "$ID", GetMessage("SALE_EDIT_RECORD")));
...
$aMenu = array(
		array(
				...
				"LINK" => "/bitrix/admin/sale_order_detail.php?ID=".$ID."&dontsave=Y&lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false)
			)
	);
...
if ($bUserCanEditOrder)
{
	$aMenu[] = array(
			...
			"LINK" => "/bitrix/admin/sale_order_edit.php?ID=".$ID."&lang=".LANGUAGE_ID.GetFilterParams("filter_", false)
		);
}
				...
				<tr>
					...
					<td width="60%">
						...
						if ($arPaySys["PSA_HAVE_RESULT"] == "Y" || strlen($arPaySys["PSA_RESULT_FILE"]) > 0)
						{
							?>
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<a href="sale_order_detail.php?ID=<?= $ID ?>&action=ps_update&lang=<?= LANG ?>&<?echo GetFilterParams("filter_", false)?>&<?= bitrix_sessid_get() ?>"><?echo GetMessage("P_ORDER_PS_STATUS_UPDATE") ?> &gt;&gt;</a>
							<?
						}
						?>
					</td>
			...
			elseif ($arPaySys["PSA_HAVE_RESULT"] == "Y" || strlen($arPaySys["PSA_RESULT_FILE"]) > 0)
			{
				...
				<tr>
					<td align="center" colspan="2">
						<a href="sale_order_detail.php?ID=<?= $ID ?>&action=ps_update&lang=<?= LANG ?>&<?= GetFilterParams("filter_", false) ?>&<?= bitrix_sessid_get() ?>"><?= GetMessage("P_ORDER_PS_STATUS_UPDATE") ?> &gt;&gt;</a>
					</td>
				</tr>
				<?
			}
			...
			<tr>
				<td colspan="2">
					...
					$dbBasket = CSaleBasket::GetList(
							...
							false,
							false,
							array("ID", "DETAIL_PAGE_URL", "NAME", "NOTES", "QUANTITY", "PRICE", "CURRENCY", "PRODUCT_XML_ID", "DISCOUNT_NAME", "DISCOUNT_VALUE", "DISCOUNT_COUPON")
						);
					?>
