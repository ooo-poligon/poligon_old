...
if (($ID>0) && ($arPaySystem = CSalePaySystem::GetByID($ID)))
	$LID = $arPaySystem["LID"];
elseif ((strlen($LID)>0) && ($arLang = CLang::GetByID($LID)))
	$ID = 0;
else
		...
		while ($arPersonType = $dbPersonType->GetNext())
		{
			...
			else
			{
				...
				if (strlen($errorMessage1) <= 0)
				{
					...
					{
						$arActFields = split(",", ${"PS_ACTION_FIELDS_LIST_".$arPersonType["ID"]});
...
$APPLICATION->SetTitle(($ID > 0) ? eregi_replace("#ID#", "$ID", GetMessage("SALE_EDIT_RECORD")) : GetMessage("SALE_NEW_RECORD"));
...
$aMenu = array(
		array(
				...
				"LINK" => "/bitrix/admin/sale_pay_system.php?lang=".LANG."&".GetFilterParams("filter_", false),
				"ICON" => "btn_list"
			)
	);
...
if ($ID > 0 && $saleModulePermissions >= "W")
{
	...
	$aMenu[] = array(
			...
			"LINK" => "/bitrix/admin/sale_pay_system_edit.php?lang=".LANG."&LID=".UrlEncode($LID)."&".GetFilterParams("filter_", false),
			"ICON" => "btn_new"
		);
...
function ActionFileChange(ind)
{
	...
	window.frames["hidden_action_frame_" + ind].location.replace('/bitrix/admin/sale_pay_system_get.php?lang=<?= htmlspecialchars($lang) ?>&file='+escape(psAction)+'&divInd='+ind);
}
	...
	for ($tpInd = 0; $tpInd < count($arPersonTypeList); $tpInd++)
	{
		...
		<tr>
			...
			<td width="60%" valign="top">
				...
				<select name="ACTION_FILE_<?= $arPersonType["ID"] ?>" OnChange="ActionFileChange(<?= $arPersonType["ID"] ?>)">
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/bitrix/admin/sale_pay_system.php?lang=".LANG."&".GetFilterParams("filter_", false)
			)
	);
?>
