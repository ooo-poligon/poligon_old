...
$aMenu = array(
		array(
				...
				"LINK" => "/bitrix/admin/sale_affiliate_plan.php?lang=".LANG."&".GetFilterParams("filter_", false),
				"ICON" => "btn_list"
			)
	);
...
if ($ID > 0)
{
	...
	$aMenu[] = array(
			...
			"LINK" => "/bitrix/admin/sale_affiliate_plan_edit.php?lang=".LANG."&".GetFilterParams("filter_", false),
			"ICON" => "btn_new"
		);
			...
			while ($arIBlock = $dbIBlockList->Fetch())
			{
				...
				foreach ($arSections as $sectionID => $arSubSection)
				{
					...
					for ($i = 0; $i < count($arSubSection); $i++)
					{
						...
						$str2 .= ", '".AddSlashes($arSubSection[$i]["NAME"])."'";
					}
					$str1 .= ");\r\n";
					$str2 .= ");\r\n";
				}
...
$tabControl->Buttons(
	array(
		...
		"back_url" => "/bitrix/admin/sale_affiliate_plan.php?lang=".LANG."&".GetFilterParams("filter_", false)
	)
);
?>
