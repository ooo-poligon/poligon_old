...
class CAllSaleAffiliatePlan
{
	...
	function Delete($ID)
	{
		...
		unset($GLOBALS["SALE_AFFILIATE_PLAN_CACHE_".$ID]);
	...
	function GetByID($ID)
	{
		...
		if (isset($GLOBALS["SALE_AFFILIATE_PLAN_CACHE_".$ID]) && is_array($GLOBALS["SALE_AFFILIATE_PLAN_CACHE_".$ID]))
		{
			return $GLOBALS["SALE_AFFILIATE_PLAN_CACHE_".$ID];
		}
		else
		{
			$strSql = 
			...
			if ($res = $db_res->Fetch())
			{
				$GLOBALS["SALE_AFFILIATE_PLAN_CACHE_".$ID] = $res;
				return $GLOBALS["SALE_AFFILIATE_PLAN_CACHE_".$ID];
			}
		}
