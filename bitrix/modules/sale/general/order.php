...
class CAllSaleOrder
{
	...
	function _Delete($ID)
	{
		...
		if ($bSuccess)
			unset($GLOBALS["SALE_ORDER_CACHE_".$ID]);
	...
	function GetByID($ID)
	{
		...
		if (isset($GLOBALS["SALE_ORDER_CACHE_".$ID]) && is_array($GLOBALS["SALE_ORDER_CACHE_".$ID]) && is_set($GLOBALS["SALE_ORDER_CACHE_".$ID], "ID"))
		{
			return $GLOBALS["SALE_ORDER_CACHE_".$ID];
		}
		else
		{
			$strSql =
			...
			if ($res = $db_res->Fetch())
			{
				$GLOBALS["SALE_ORDER_CACHE_".$ID] = $res;
				return $res;
			}
		}
	...
	function OnBeforeCurrencyDelete($currency)
	{
		...
		/*
		$cnt = CSaleOrder::GetList(array(), array("CURRENCY" => $currency), array());
		if ($cnt > 0)
			return False;
	...
	function DeliverOrder($ID, $val, $recurringID = 0)
	{
		global $DB, $USER;
	...
	function UnLock($ID)
	{
		...
		if (($userRights >= "W") || ($arOrder["LOCKED_BY"] == $GLOBALS["USER"]->GetID()))
		{
			...
			if (!CSaleOrder::Update($ID, $arFields))
				return False;
			else
				return True;
		}
