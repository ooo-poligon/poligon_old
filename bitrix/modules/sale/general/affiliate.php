...
class CAllSaleAffiliate
{
	...
	function Delete($ID)
	{
		...
		unset($GLOBALS["SALE_AFFILIATE_CACHE_".$ID]);
	...
	function GetByID($ID)
	{
		...
		if (isset($GLOBALS["SALE_AFFILIATE_CACHE_".$ID]) && is_array($GLOBALS["SALE_AFFILIATE_CACHE_".$ID]))
		{
			return $GLOBALS["SALE_AFFILIATE_CACHE_".$ID];
		}
		else
		{
			$strSql = 
			...
			if ($res = $db_res->Fetch())
			{
				$GLOBALS["SALE_AFFILIATE_CACHE_".$ID] = $res;
				return $GLOBALS["SALE_AFFILIATE_CACHE_".$ID];
			}
		}
	...
	function GetAffiliate($affiliateID = 0)
	{
		...
		if ($affiliateID > 0)
		{
			...
			$GLOBALS["APPLICATION"]->set_cookie("SALE_AFFILIATE", $affiliateID, (($cookieTime <= 0) ? 0 : $cookieTime * 24 * 60 * 60));
		}
