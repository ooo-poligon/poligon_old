...
class CAllSaleExport
{
	...
	function Delete($ID)
	{
		...
		unset($GLOBALS["SALE_EXPORT_CACHE_".$ID]);
	...
	function GetByID($ID)
	{
		...
		if (isset($GLOBALS["SALE_EXPORT_CACHE_".$ID]) && is_array($GLOBALS["SALE_EXPORT_CACHE_".$ID]) && is_set($GLOBALS["SALE_EXPORT_CACHE_".$ID], "ID"))
		{
			return $GLOBALS["SALE_EXPORT_CACHE_".$ID];
		}
		else
		{
			$strSql =
			...
			{
				$GLOBALS["SALE_EXPORT_CACHE_".$ID] = $arResult;
				return $arResult;
			}
		}
	...
	function ExportOrders2Xml($arFilter = Array(), $nTopCount = 0, $currency = "")
	{
		...
		?>
		<<?=GetMessage("SALE_EXPORT_COM_INFORMATION")?> <?=GetMessage("SALE_EXPORT_SHEM_VERSION")?>="2.03" <?=GetMessage("SALE_EXPORT_SHEM_DATE_CREATE")?>="<?=date("Y-m-d")?>T<?=date("G:i:s")?>">
		<?
