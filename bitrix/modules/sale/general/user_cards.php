...
class CAllSaleUserCards
{
	...
	function IdentifyCardType($ccNum)
	{
		...
		if (ereg('^4(.{12}|.{15})$', $ccNum))
			return 'VISA';
		elseif (ereg('^5[1-5].{14}$', $ccNum))
			return 'MASTERCARD';
		elseif (ereg('^3[47].{13}$', $ccNum))
			return 'AMEX';
		elseif (ereg('^3(0[0-5].{11}|[68].{12})$', $ccNum))
			return 'DINERS';
		elseif (ereg('^6011.{12}$', $ccNum))
			return 'DISCOVER';
		elseif (ereg('^(3.{15}|(2131|1800).{11})$', $ccNum))
			return 'JCB';
		elseif (ereg('^2(014|149).{11})$', $ccNum))
			return 'ENROUTE';
		else
			return "N";
	}
