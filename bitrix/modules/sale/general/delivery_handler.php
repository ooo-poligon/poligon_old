...
WHERE HID IN (".$strKeys.")";
	...
	function __getRegisteredHandlers()
	{
		...
		if ($rsHandlersList->SelectedRowsCount() > 0)
		{
			...
			{
				$arHandlersList[] = ExecuteModuleEvent($arHandler);
			}
		}
...
";
		if (count($arOrder) > 0)
		{
			...
			$bFirst = true;
			foreach ($arOrder as $SORT => $SORT_DIR)
			{
				if ($bFirst)
					$bFirst = false;
				else
		...
		foreach ($arHandlersList as $key => $arHandler)
		{
			if (!array_key_exists($arHandler["SID"], $arLoadedHandlersMap))
			{
		...
		if (is_array($arFilter["COMPABILITY"]) && !empty($arHandlersList))
		{
			foreach ($arHandlersList as $key => $arHandler)
			{
	...
	function __executeCalculateEvents($SID, $profile, $arOrder, $arReturn)
	{
		...
		foreach ($arEventsList as $event)
		{
			...
			if ($rsEventHandlersList->SelectedRowsCount() > 0)
			{
				...
				{
					$arReturnTmp = ExecuteModuleEvent($arEventHandler, $SID, $profile, $arOrder, $arReturn);
