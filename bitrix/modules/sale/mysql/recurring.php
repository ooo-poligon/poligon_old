...
class CSaleRecurring extends CAllSaleRecurring
{
	function GetByID($ID)
	{
		...
		if (isset($GLOBALS["SALE_RECURRING_CACHE_".$ID]) && is_array($GLOBALS["SALE_RECURRING_CACHE_".$ID]) && is_set($GLOBALS["SALE_RECURRING_CACHE_".$ID], "ID"))
		{
			return $GLOBALS["SALE_RECURRING_CACHE_".$ID];
		}
		else
		{
			$strSql = 
			...
			if ($res = $db_res->Fetch())
			{
				$GLOBALS["SALE_RECURRING_CACHE_".$ID] = $res;
				return $res;
			}
		}
