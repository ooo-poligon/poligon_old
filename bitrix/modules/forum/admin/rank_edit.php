...
/*
	if ($ID>0)
	{
		if (!CForumRank::CanUserUpdateRank($ID, $USER->GetUserGroupArray()))
			$strErrorMessage .= "Ќе достаточно прав дл€ звани€ с кодом ".$ID.". \n";
	}
	else
	{
		if (!CForumRank::CanUserAddRank($USER->GetUserGroupArray()))
			$strErrorMessage .= "Ќе достаточно прав дл€ добавлени€ звани€. \n";
	}
*/
...
if ($ID>0)
{
	...
	$db_rank->ExtractFields("str_", True);
}
...
<table border="0" cellspacing="1" cellpadding="3" width="100%" class="edittable">
	...
	<tr>
		...
		<td valign="top" align="left" class="tablebody" width="50%">
			<input type="text" class="typeinput" name="MIN_NUM_POSTS" value="<?echo $str_MIN_NUM_POSTS ?>" size="10">
		</td>
	</tr>
	...
	for ($i = 0; $i<count($arSysLangs); $i++):
		...
		$str_NAME = htmlspecialchars($arRankLang["NAME"]);
		if ($bInitVars)
		{
		...
		<tr>
			...
			<td valign="top" align="left" class="tablebody" width="50%">
				<input type="text" class="typeinput" name="NAME_<?echo $arSysLangs[$i] ?>" value="<?echo $str_NAME ?>" size="30">
			</td>
		</tr>
	<?endfor;?>
