<?
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/forum/general/private_message.php");
	...
	function GetListEx($arOrder = Array("ID"=>"ASC"), $arFilter = Array(), $bCount = false, $iNum = 0)
	{
		...
		$strSql = 
			"SELECT 
				...
				CASE
			         WHEN ((FUU.SHOW_NAME='Y') AND (LENGTH(TRIM(CONCAT(UU.NAME,UU.LAST_NAME)))>0)) THEN trim(CONCAT_WS(' ',UU.NAME, UU.LAST_NAME))
			         ELSE UU.LOGIN
