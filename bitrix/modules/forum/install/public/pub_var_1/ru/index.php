...
Впервые на нашем форуме? Важная информация для новых и постоянных посетителей - 
<a href="regulation.php">правила форумов</a>. 
					</font></td>
		</tr>
	</table>
	</td>
...
<p class="text"><?echo $db_Forum->NavPrint("Форумы")?></p>
	...
	while ($db_Forum->NavNext(true, "f_", false)):
		...
		<tr class="forumbody">
			<td align="center" class="forumbodytext" valign="top">
				...
				{
					?><a href="read.php?FID=<?echo $f_ID;?>&TID=<?echo $FirstUnreadedTopicID?>&MID=<?echo $FirstUnreadedMessageID?>#message<?echo $FirstUnreadedMessageID?>"><img src="/bitrix/images/forum/f_norm.gif" width="17" height="11" alt="Есть новые сообщения!" border="0" vspace="3"></a><?
				}
				else
				{
					?><img src="/bitrix/images/forum/f_norm_no.gif" width="17" height="11" alt="Нет новых сообщений" border="0" vspace="3"><?
				}
				?><br>
			</td>
			...
			<td class="forumbodytext" valign="top">
				...
				<?if (strlen($f_TITLE)>0):?>
					<font class="forumheadcolor">тема:</font> <a href="read.php?FID=<?echo $f_ID;?>&TID=<?echo $f_TID;?>&MID=<?echo $f_MID;?>#message<?echo $f_MID;?>"><?echo (strlen($f_TITLE)>23) ? substr($f_TITLE, 0, 20)."..." : $f_TITLE;?></a><br>
				<?endif;?>
				<?if (strlen($f_LAST_POSTER_NAME)>0):?>
					<font class="forumheadcolor">автор:</font> <?echo (IntVal($f_LAST_POSTER_ID)>0)?"<a href=\"view_profile.php?UID=".$f_LAST_POSTER_ID."\">":""?><?echo $f_LAST_POSTER_NAME?><?echo (IntVal($f_LAST_POSTER_ID)>0)?"</a>":""?>
				<?endif;?>
			</td>
		</tr>
		<?
	...
	<tr valign="top" align="center">
		<td>
			<table width="100%" border="0" cellspacing="0" cellpadding="3" class="forumbody">
				<tr valign="middle"> 
					<td style="padding-right:0px; padding-left:5px;"><font class="forumbodytext">Логин:</font></td>
					<td>
<input type="text" name="USER_LOGIN" maxlength="50" size="13" value="<?echo htmlspecialchars(${COption::GetOptionString("main", "cookie_name", "BITRIX_SM")."_LOGIN"})?>">
					...
					<td style="padding-right:0px; padding-left:5px;"><font class="forumbodytext">Пароль:</font></td>
					<td>
