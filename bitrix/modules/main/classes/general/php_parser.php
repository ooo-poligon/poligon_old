...
class PHPParser
{
	...
	function ReturnPHPStrRec($arVal, $level, $comm = "")
	{
		...
		if (is_array($arVal))
		{
			...
			foreach ($arVal as $key => $value)
				$result .= $pref."\t".((IntVal($key)."|" == $key."|") ? $key : /*"\"".$key."\""*/PHPParser::PreparePHP($key))."\t=>\t".PHPParser::ReturnPHPStrRec($value, $level + 1);
			$result .= $pref."),\r\n";
		}
		else
		{
	...
	function ReturnPHPStr2($arVals, $arParams)
	{
		$res = "";
		$i = 0;
		...
		foreach($arVals as $key => $val)
		{
			...
			$res .= "\t\"".$key."\"\t=>\t";
			$res .= PHPParser::ReturnPHPStrRec($val, 2, $comm);
		}
