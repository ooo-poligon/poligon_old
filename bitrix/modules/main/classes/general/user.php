	...
	*/
	...
	function Login($login, $password, $remember="N", $password_original="Y")
	{
		...
		if($bOk)
		{
			...
			if($user_id<=0)
			{
				...
				$result = $DB->Query($strSql, false, "FILE: ".__FILE__."<br> LINE: ".__LINE__);
				$pass = ($arParams["PASSWORD_ORIGINAL"]=="Y") ? md5($arParams["PASSWORD"]) : $arParams["PASSWORD"];
				if(($arUser = $result->Fetch()) && $pass==$arUser["PASSWORD"])
				{
					if($arUser["ACTIVE"] == "Y")
						$user_id = $arUser["ID"];
					else
	...
	function ChangePassword($LOGIN, $CHECKWORD, $PASSWORD, $CONFIRM_PASSWORD, $SITE_ID=false)
	{
		...
		if($bOk)
		{
			...
			if(strlen($res["CHECKWORD"])<=0 || $res["CHECKWORD"] != $arParams["CHECKWORD"])
			...
			$obUser = new CUser;
			$res = $obUser->Update($ID, Array("PASSWORD"=>$arParams["PASSWORD"], "CHECKWORD"=>$checkword));
	...
	function Register($USER_LOGIN, $USER_NAME, $USER_LAST_NAME, $USER_PASSWORD, $USER_CONFIRM_PASSWORD, $USER_EMAIL, $SITE_ID = false, $captcha_word = "", $captcha_sid = 0)
	{
		...
		if (COption::GetOptionString("main", "captcha_registration", "N") == "Y")
		{
			...
			{
				$APPLICATION->ThrowException(GetMessage("MAIN_FUNCTION_REGISTER_CAPTCHA"));
				return Array("MESSAGE"=>GetMessage("MAIN_FUNCTION_REGISTER_CAPTCHA"), "TYPE"=>"ERROR");
			}
		}
		...
		if($bOk)
		{
			...
			if($ID = $this->Add($arFields))
			{
				...
				$event = new CEvent;
				$event->Send("NEW_USER", $arFields["SITE_ID"], $arFields);
	...
	function SimpleRegister($USER_EMAIL, $SITE_ID = false, $captcha_word = "", $captcha_sid = 0)
	{
		...
		if($bOk)
		{
			...
			if($ID = $this->Add($arFields))
			{
				...
				$event->Send("NEW_USER", $arFields["SITE_ID"], $arFields);
				CUser::SendUserInfo($ID, $arFields["SITE_ID"], GetMessage("INFO_REQ"));
				$result_message = Array("MESSAGE"=>GetMessage("USER_REGISTER_OK"), "TYPE"=>"OK");
			}
			else
				$result_message = Array("MESSAGE"=>$this->LAST_ERROR, "TYPE"=>"ERROR");
	...
	function CheckFields(&$arFields, $ID=false)
	{
		...
		if($bInternal)
		{
			...
			if(is_set($arFields, "PASSWORD") && strlen($arFields["PASSWORD"])<6)
				$this->LAST_ERROR .= GetMessage("MIN_PASSWORD1")."<br>";
	...
	function Update($ID, $arFields)
	{
		...
		else
		{
			...
			$strUpdate = $DB->PrepareUpdate("b_user", $arFields, "main");
	...
	function GetGroupPolicy($iUserId)
	{
		...
		if(is_set($arPOLICY_CACHE, $iUserId))
			return $arPOLICY_CACHE[$iUserId];
		...
		if(count($arPOLICY_CACHE)<=10)
			$arPOLICY_CACHE[$iUserId] = $arPolicy;
