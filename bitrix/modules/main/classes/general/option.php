...
class CAllOption
{
	...
	function SetOptionString($module_id, $name, $value="", $desc=false, $site="")
	{
		...
		else
		{
			$DB->Query(
				...
				"VALUES(".($site==""?"NULL":"'".$DB->ForSQL($site, 2)."'").", '".$DB->ForSql($module_id, 50)."', '".$DB->ForSql($name, 50)."', '".$DB->ForSql($value, 2000)."', '".$DB->ForSql($desc, 255)."') "
				);
		}
		...
		global $MAIN_OPTIONS;
		if(isset($MAIN_OPTIONS[$site][$module_id]))
		...
		while ($arEvent = $events->Fetch())
			ExecuteModuleEvent($arEvent, $value);
	...
	function RemoveOption($module_id, $name="", $site=false)
	{
		...
		if($module_id == "main")
			$DB->Query("DELETE FROM b_option WHERE MODULE_ID='main' AND NAME NOT LIKE '~%' AND NAME<>'crc_code' AND NAME<>'admin_passwordh' AND NAME<>'server_uniq_id' AND NAME<>'PARAM_MAX_SITES' ".$strSqlWhere);
		else
