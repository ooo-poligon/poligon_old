...
//						$res .= '<script type="text/javascript">loaded=true;</script>';
	...
	function ConvertCharset($string, $charset_in, $charset_out)
	{
		...
		if(!$res)
			$this->ThrowException($res->errorMessage, "ERR_CHAR_BX_CONVERT");
...
class CAllFile
{
	...
	{
		global $DB;
		$DB->Query("UPDATE b_file SET DESCRIPTION='".$DB->ForSql($desc, 255)."' WHERE ID=".$ID);
		CFile::CleanCache($ID);
	}
