		...
		if (file_exists($_SERVER["DOCUMENT_ROOT"].$componentPath."/.parameters.php"))
		{
			...
			for ($i = 0, $cnt = count($arParamKeys); $i < $cnt; $i++)
			{
					...
					foreach ($arSavedParams as $keyTmp => $valueTmp)
					{
						if ($keyTmp == "CACHE_TIME")
						{
							$arComponentParameters["PARAMETERS"]["CACHE_TYPE"] = array(
								...
								"TYPE" => "LIST",
								"VALUES" => array("A" => GetMessage("COMP_PROP_CACHE_TYPE_AUTO"), "Y" => GetMessage("COMP_PROP_CACHE_TYPE_YES"), "N" => GetMessage("COMP_PROP_CACHE_TYPE_NO")),
								"DEFAULT" => "A",
								"ADDITIONAL_VALUES" => "N"
							);
				...
				elseif ($arParamKeys[$i] == "AJAX_MODE")
				{
					...
					/*
					$arComponentParameters["PARAMETERS"]["AJAX_OPTIONS"] = array(
						"PARENT" => "AJAX_SETTINGS",
						"NAME" => GetMessage("COMP_PROP_AJAX_OPTIONS"),
						"TYPE" => "LIST",
						"MULTIPLE" => "Y",
						"VALUES" => array(
							"NOSHADOW" => GetMessage("COMP_PROP_AJAX_OPTIONS_NOSHADOW"),
							"NOJUMP" => GetMessage("COMP_PROP_AJAX_OPTIONS_NOJUMP"),
							"NOSTYLE" => GetMessage("COMP_PROP_AJAX_OPTIONS_NOSTYLE"),
						),
						"DEFAULT" => array('NOJUMP'),
						"ADDITIONAL_VALUES" => "N"
					);
					*/
				}
			...
			if(
				...
				&& (strpos($componentPath, "/photogallery")!==false)
			)
			{
				//Create directories
