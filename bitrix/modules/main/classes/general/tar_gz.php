...
class CArchiver
{
	...
	function _addFile($strFilename, &$arHeaders, $strAddPath, $strRemovePath)
	{
		...
		if (is_file($strFilename))
		{
			...
			{
				if ($this->_bCompress)
					@gzseek($dfile, $this->file_pos-512);
				else
					@fseek($dfile, $this->file_pos-512);
			}

			while ((($v_buffer = fread($dfile, 512)) != '') && (($istime = ((getmicrotime() - $this->start_time) < round($this->max_exec_time))) || !$this->stepped ))
			{
