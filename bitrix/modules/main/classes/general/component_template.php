...
class CBitrixComponentTemplate
{
	...
	function &GetCachedData()
	{
		...
		if (StrLen($this->__folder) > 0)
			if (file_exists($_SERVER["DOCUMENT_ROOT"].$this->__folder."/style.css"))
	...
	function Init(&$component, $siteTemplate = false)
	{
		...
		if (!preg_match("#^([A-Za-z0-9_.-]+)(/[A-Za-z0-9_.-]+)?$#i", $this->__name))
			$this->__name = ".default";
	...
	{
		if (StrLen($this->__folder) > 0)
			if (file_exists($_SERVER["DOCUMENT_ROOT"].$this->__folder."/script.js"))
				echo "<script src=\"".$this->__folder."/script.js\" type=\"text/javascript\"></script>";
	}
