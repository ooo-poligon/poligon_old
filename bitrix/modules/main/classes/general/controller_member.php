...
class CControllerClient
{
	...
	function OnExternalLogin($arParams)
	{
		...
		if(substr(strtolower($arParams["LOGIN"]), 0, strlen($prefix)) == $prefix)
		{
			$site = $prefix;
	...
	function AuthorizeAdmin($arParams = Array())
	{
		...
		else
		{
			...
			$ADMIN_ID = CControllerClient::UpdateUser($arUser);
		}
	...
	function SetModules($arModules)
	{
		...
		foreach($arModules as $module_id=>$status)
		{
			if(!file_exists($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/modules/".$module_id."/install/index.php"))
				continue;
			require_once($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/modules/".$module_id."/install/index.php");
			$oModule = new $module_id;
...
class __CControllerPacketRequest extends __CControllerPacket
{
	...
	function InitFromRequest()
	{
		...
		{
			//if(count($_POST)<=0)
			//	$this->arParameters = unserialize($_REQUEST['parameters']);
			//else
			$this->arParameters = unserialize(base64_decode($_REQUEST['parameters']));
		}
	...
	function Check()
	{
		$hash = $this->operation."|".serialize($this->arParameters)."|".$this->secret_id;
		//AddMessage2Log($hash);
	...
	function Send($url, $page)
	{
		...
		{
			$server_name = 'ssl://'.substr($server_name, 8);
			$server_port = 443;
		}
		...
		$conn = @fsockopen($server_name, $server_port, $errno, $errstr, 30);
		if(!$conn)
		{
...
class __CControllerPacketResponse extends __CControllerPacket
{
	...
	function ParseResult($result)
	{
		...
		$this->arParameters = unserialize(base64_decode(urldecode($ar_result['parameters'])));
