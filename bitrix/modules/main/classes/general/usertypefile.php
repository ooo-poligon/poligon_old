...
class CUserTypeFile
{
	...
	function GetEditFormHTML($arUserField, $arHtmlControl)
	{
		...
		return '<input type="hidden" name="'.$strOldIdName.'" value="'.$arHtmlControl["VALUE"].'">'.
			CFile::InputFile($arHtmlControl["NAME"], $arUserField["SETTINGS"]["SIZE"], $arHtmlControl["VALUE"], false, 0, "", "", 0, "", 'value="'.$arHtmlControl["VALUE"].'"').
			'<br>'.
	...
	function CheckFields($arUserField, $value)
	{
		...
		//Extention check
		$bImageOnly = False;
		$arImageExtentions = explode(",", strtolower(CFile::GetImageExtensions()));
		if(is_array($arUserField["SETTINGS"]["EXTENSIONS"]))
		{
			...
			foreach($arUserField["SETTINGS"]["EXTENSIONS"] as $ext => $tmp_val)
			{
				...
				if(!in_array(strtolower($ext), $arImageExtentions))
				{
					$bImageOnly = False;
					break;
				}
		...
		elseif(is_array($arUserField["SETTINGS"]["EXTENSIONS"]) && count($arUserField["SETTINGS"]["EXTENSIONS"]))
		{
			foreach($arUserField["SETTINGS"]["EXTENSIONS"] as $ext => $tmp_val)
				$arUserField["SETTINGS"]["EXTENSIONS"][$ext] = $ext;
