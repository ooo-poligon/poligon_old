...
class CAjax
{
	...
	function GetComponentID($componentName, $componentTemplate)
	{
		if(function_exists("debug_backtrace"))
		{
			...
			{
				if ($aTrace[$i]['function'] == 'IncludeComponent' && $aTrace[$i]['class'] == 'CAllMain')
				{
					$trace_current = $i;
					break;
				}
			...
			if($iSrcLine > 0 && $sSrcFile <> "")
			{
				...
				$doc_root = rtrim(str_replace(Array("\\\\", "//", "\\"), Array("\\", "/", "/"), $_SERVER["DOCUMENT_ROOT"]), "\\/");
