...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_REQUEST["save"]) && $strWarning == "")
{
	...
	else
	{
		...
		?>
			<script>
				jsPopup.ShowDialog("/bitrix/admin/public_file_edit.php?lang=<?=$lang?>"+(jsUtils.IsEditor() ? "" : "&noeditor=Y")+"&path=<?=urlencode($pathToEdit)?>&site=<?=$site?>&back_url=<?=urlencode($relativePath)?>", {width: 770, height: 570, resize: false});
				jsUtils.addCustomEvent("OnBeforeCloseDialog", function() {window.location = "<?=CUtil::JSEscape($relativePath)?>"});
			</script>
		<?
		else:
