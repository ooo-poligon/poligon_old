	...
	if($strWarning == "")
	{
		...
		for ($i = 0, $cnt = count($arComponents); $i < $cnt; $i++)
		{
			...
			if ($nLineFrom <= $src_line && $nLineTo >= $src_line)
			{
				...
				{
					$arValues = $arComponents[$i]["DATA"]["PARAMS"];
...
if($strWarning == "")
{
	...
	$arTemplateParameters = CComponentUtil::GetTemplateProps($_GET["component_name"], $_GET["component_template"], $_GET["template_id"], $arValues);
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && $_GET["action"] == "save" && $arComponent !== false && $arComponentDescription !== false)
	{
		...
		else
		{
			...
			$params = PHPParser::ReturnPHPStr2($aPostValues, $arParameters);
			$code =  ($arComponent["DATA"]["VARIABLE"]? $arComponent["DATA"]["VARIABLE"]."=":"").
				"\$APPLICATION->IncludeComponent(\"".$_GET["component_name"]."\", ".
...
?>
	...
	<tr<?if($bHidden) echo ' style="display:none"'?>>
		...
		<td>
			<select name="NEW_COMPONENT_TEMPLATE">
				<?
				for ($i = 0, $cnt = count($arComponentTemplates); $i < $cnt; $i++)
				{
					...
					?><option value="<?= htmlspecialchars($arComponentTemplates[$i]["NAME"]) ?>"<?if ($arComponentTemplates[$i]["NAME"]==$_GET["component_template"] || StrLen($_GET["component_template"]) <= 0 && $arComponentTemplates[$i]["NAME"]==".default") echo " selected";?>><?= htmlspecialchars($showTemplateName.": ".$arComponentTemplates[$i]["NAME"]) ?></option><?
				}
				?>
			</select>
		</td>
