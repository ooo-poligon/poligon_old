...
if($strWarning == "")
{
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["action"] == "save" && $arComponent !== false && $arComponentDescription !== false)
	{
		$_POST["TEMPLATE_NAME"] = Trim($_POST["TEMPLATE_NAME"]);
		if (StrLen($_POST["TEMPLATE_NAME"]) <= 0)
			$_POST["TEMPLATE_NAME"] = ".default";

		if ($_POST["SITE_TEMPLATE"] != $_GET["template_id"] && $_POST["SITE_TEMPLATE"] != ".default")
			$_POST["USE_TEMPLATE"] = "N";

		if (CComponentUtil::CopyTemplate($arComponent["DATA"]["COMPONENT_NAME"], $arComponent["DATA"]["TEMPLATE_NAME"], ((StrLen($templateSiteTemplate) > 0) ? $templateSiteTemplate : False), $_POST["SITE_TEMPLATE"], $_POST["TEMPLATE_NAME"], False))
		{
			if ($_POST["USE_TEMPLATE"] == "Y")
			{
				//echo "<pre>";print_r($arComponent["DATA"]["PARAMS"]);echo "</pre>";
				//die();
				//echo "<pre>";print_r($arParameters);echo "</pre>";
				$params = PHPParser::ReturnPHPStr2($arComponent["DATA"]["PARAMS"], $arParameters);

				$code = ($arComponent["DATA"]["VARIABLE"]?$arComponent["DATA"]["VARIABLE"]."=":"")."\$APPLICATION->IncludeComponent(\"".$arComponent["DATA"]["COMPONENT_NAME"]."\", \"".$_POST["TEMPLATE_NAME"]."\", Array(\r\n\t".$params."\r\n\t)".((StrLen($arComponent["DATA"]["PARENT_COMP"]) > 0) ? ",\r\n".$arComponent["DATA"]["PARENT_COMP"] : "")."\r\n);";
					...
					if ($component->InitComponent($arComponent["DATA"]["COMPONENT_NAME"], $_POST["TEMPLATE_NAME"]))
					{
						if ($component->InitComponentTemplate($_REQUEST["edit_file"], $_POST["SITE_TEMPLATE"]))
						{
							...
							if (!is_null($template))
							{
								...
								//$strJSText = 'jsPopup.ShowDialog("/bitrix/admin/public_file_edit_src.php?lang='.LANGUAGE_ID.'&site='.SITE_ID.'&back_url='.urlencode($_REQUEST["back_path"]).'&path='.urlencode($template->GetFile()).'", {\'width\':\'770\', \'height\':\'570\', \'resize\':false});';
								//$strJSText = 'window.location.href = "/bitrix/admin/fileman_file_edit.php?full_src=Y&site='.SITE_ID.'&lang='.LANGUAGE_ID.'&back_url='.urlencode($_REQUEST["back_path"]).'&path='.urlencode($template->GetFile()).'";';
							}
						}
					}
				}
...
<?//echo '<pre>'; print_r($arTemplateParameters); print_r($arComponent); print_r($arComponentDescription); echo '</pre>';?>
...
?>
	<tr>
		...
		<td>
<?
if($arComponent["DATA"]["PARENT_COMP"] == ""):
