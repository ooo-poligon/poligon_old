...
$popupWindow = new CJSPopup(GetMessage("PAGE_DELETE_WINDOW_TITLE"));
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_REQUEST["save"]) && $strWarning == "")
{
	...
	if ($success)
	{
		...
		$popupWindow->Close($bReload = true, $back_url);
	}
	else
...
<?if (IsModuleInstalled("fileman")):?>
<form name="bx_page_delete" style="margin:2px; padding:2px;">
...
&nbsp;&nbsp;&nbsp;<input name="btn_popup_close" type="button" value="<?=GetMessage("PAGE_DELETE_BUTTON_NO")?>" onclick="jsPopup.CloseDialog()" title="<?=GetMessage("PAGE_DELETE_BUTTON_NO")?>" />
