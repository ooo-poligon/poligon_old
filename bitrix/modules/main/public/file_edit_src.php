...
?>
<script>
jsPopup.ShowError('<?=CUtil::JSEscape($strWarning)?>');
</script>
<?
		*/
		}
...
$obJSPopup->ShowTitlebar($bEdit ? GetMessage("FILEMAN_FILEEDIT_PAGE_TITLE")." \"".$arParsedPath["LAST"]."\"" : GetMessage("FILEMAN_NEWFILEEDIT_TITLE"));
...
?>
	<textarea id="filesrc" name="filesrc" style="height: <?=intval($arPos['filesrc_height'])?>px; width: <?=intval($arPos['filesrc_width'])?>px;"><?echo htmlspecialchars($filesrc)?></textarea>
	<script type="text/javascript">
		document.getElementById('filesrc').BXResizeCacheID = 'filesrc';
		jsPopup.addAdditionalResize('filesrc');
	</script>
	<?endif?>
<?
