...
if($REQUEST_METHOD=="POST" && ($save<>"" || $apply<>"") && check_bitrix_sessid())
{
	...
	if($isAdmin)
	{
		$arFields["COMMON"] = ($COMMON == "Y"? "Y" : "N");
		$arFields["USER_ID"] = ($arFields["COMMON"] == "Y"? false : $USER_ID);
		$arFields["MODULE_ID"] = ($arFields["COMMON"] == "Y" && $MODULE_ID <> ""? $MODULE_ID : false);
	}
...
if(strpos($_REQUEST["name"], "%u") !== false)
	$_REQUEST["name"] = $GLOBALS["APPLICATION"]->UnJSEscape($_REQUEST["name"]);
