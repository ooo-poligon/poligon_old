...
if($REQUEST_METHOD == "POST" && ($edit_php || $lpa))
{
	$CONTENT = decode($CONTENT);
	$STYLES = decode($STYLES);
	$TEMPLATE_STYLES = decode($TEMPLATE_STYLES);
	$strWarning = "";
...
function decode($str)
{
	$str = $GLOBALS["APPLICATION"]->ConvertCharset($str, 'UTF-8', 'Windows-1251');
	$str = $GLOBALS["APPLICATION"]->UnJSEscape($str);
	return $str;
}
