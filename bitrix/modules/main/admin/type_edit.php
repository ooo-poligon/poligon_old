...
if($REQUEST_METHOD=="POST" && (strlen($save)>0 || strlen($apply)>0) && $isAdmin && check_bitrix_sessid())
{
	...
	{
		$arType = $_POST["FIELDS"][$idLang];
		$arType["LID"] = $idLang;
		$arType["EVENT_NAME"] = $res["EVENT_NAME"];
	...
	$context = new CAdminContextMenu(
		array(
			...
			array(
				...
				"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/bitrix/admin/type_admin.php?ID=".$arParams["EVENT_NAME"]."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."&action=delete';",
			...
			array(
				...
				"LINK"	=> "/bitrix/admin/message_admin.php?find_event_type=".$arParams["EVENT_NAME"]."&amp;lang=".LANGUAGE_ID,
				"TITLE"	=> GetMessage("TEMPLATE_LIST_TITLE"),
				"ICON"	=> "btn_list"
			),
			array(
				"TEXT"	=> GetMessage("MAIN_NEW_TEMPLATE"),
				"LINK"	=> "/bitrix/admin/message_edit.php?EVENT_NAME=".$arParams["EVENT_NAME"]."&type=this&lang=".LANGUAGE_ID,
