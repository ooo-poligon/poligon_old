				...
				if($oArchiver->extractFiles($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/templates/".$ID))
				{
					...
					if(strlen($SITE_ID)>0 && $SITE_ID!="NOT_REF")
					{
						...
						if($ar_site = $db_site->Fetch())
						{
							...
							$arFields["TEMPLATE"] = $arTemplates;
							$arFields["NAME"] = $ar_site["NAME"];
							$ob_site = new CSite();
...
?>
	...
	<tr valign="top">
		<td width="40%"><?echo GetMessage("MAIN_TEMPLATE_LOAD_ID")?></td>
		<td width="60%"><input type="text" name="ID" Id="ID" size="20" maxlength="255" value="<? echo $str_ID?>"></td>
	</tr>
	<tr valign="top">
		<td width="40%"><?echo GetMessage("MAIN_TEMPLATE_LOAD_SITE_ID")?></td>
		<td width="60%"><?=CSite::SelectBox("SITE_ID", $str_SITE_ID, GetMessage("MAIN_TEMPLATE_LOAD_SITE_ID_N"))?></td>
	</tr>
	<tr valign="top">
		<td width="40%"><?echo GetMessage("MAIN_TEMPLATE_LOAD_GOTO_EDIT")?></td>
		<td width="60%"><input type="checkbox" name="goto_edit" value="Y"></td>
	</tr>
<?
$tabControl->Buttons();
?>
