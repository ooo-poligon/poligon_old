...
if ($ID>0)
{
	...
	if($isAdmin)
	{
		$aMenu[] = array(
			...
			"LINK"	=> "/bitrix/admin/lang_edit.php?lang=".LANGUAGE_ID.htmlspecialchars("&COPY_ID=").$str_LID,
			"TITLE"	=> GetMessage("MAIN_COPY_RECORD_TITLE"),
			"ICON"	=> "btn_copy"
			);
		$aMenu[] = array(
			"TEXT"	=> GetMessage("MAIN_DELETE_RECORD"),
			"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/bitrix/admin/lang_admin.php?ID=".$str_LID."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."&action=delete';",
