...
if($bFormValues)
{
	$aUserOpt = array(
		...
		"messages" => array("support"=>$_REQUEST["messages_support"]),
	);
}
else
{
...
if($theme["PREVIEW"] <> "")
{
	...
	if(file_exists($_SERVER["DOCUMENT_ROOT"].$previewName))
	{
		...
		if($aSize !== false)
		{
			...
			echo '<a title="'.GetMessage("user_sett_screen").'" href="'.htmlspecialchars($previewName).'" onclick="ImgShw(\''.htmlspecialchars($previewName).'\', '.$aSize[0].', '.$aSize[1].', \'\'); return false;"><img src="/bitrix/images/main/preview.gif" width="16" height="16" alt="'.GetMessage("user_sett_screen").'" border="0"></a>';
		}
	}
}
?>
