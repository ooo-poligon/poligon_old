...
$def_lang .= (strlen($def_lang) != 2) ? "ru" : "";

@set_time_limit(0);
$lang = (strlen(@$_REQUEST["lang"]) > 0) ? @$_REQUEST["lang"] : $def_lang;
...
$mArr_en = array(
			...
			"TITLE1" => "Step 1: Archive Extracting",
			"TITLE_PROCESS1" => "Step 1: Extracting an archive...",
			"TITLE_PROCESS2" => "Step 2: Restoring database...",
			"TITLE2" => "Step 2: Database restoring",
			...
			"BUT_DELL" => "Delete",
			"FINISH_ERR_DELL" => "Failed to delete all files! You should delete them manually",
...
<script language="JavaScript">
	...
	function reloadPage(val, lang)
	{
			document.getElementById('Step_id').value = val;
			document.getElementById('restore').action='restore.php?='+lang;
...
if($Step == 1)
{
	?>
		...
		<tr>
			...
			<td class="tablebody1" align="right" nowrap="nowrap"><?= getMsg("SELECT_LANG", $lang)?></td>
			<td class="tablebody1" width="240" onChange="return selectLang()"><select class="selectitem" name="lang" id="lang_id"><option <? if($lang == "en") echo "SELECTED";?> value="en">EN<option value="ru" <? if($lang == "ru") echo "SELECTED";?>>RU</select></td>
			<td class="tablebody1" width="19">&nbsp;</td>
			<td class="tableborder1" width="1"></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="tablebody1" width="240"><input type="button" class="selectitem" id="start_button" value="<?= getMsg("BUT_TEXT1", $lang)?>" onClick="if(document.restore.arc_name.value=='') alert('<?= getMsg("ERR_NO_ARC", $lang)?>'); else return reloadPage(2, '<?= $lang?>');"></td>
			<td class="tablebody1" width="19">&nbsp;</td>
			<td class="tableborder1" width="1"></td>
		</tr>
		<tr>
...
elseif($Step == 4)
{
	...
	if(!$Warn_a || !$Warn_b)
		echo showMsg(getMsg("FINISH", $lang), getMsg("FINISH_ERR_DELL", $lang));
	...
	function Connect()
	{
		...
		if (@$_REQUEST["create_db"]=="Y")
		{
			if(!@mysql_query("CREATE DATABASE ".@$_REQUEST["db_name"], $this->db_Conn))
				echo showMsg(getMsg("ERR_CREATE_DB", $lang), "".mysql_error());
		...
		$bNeedCP = False;
		$dbQueryRes =  @mysql_query("select VERSION() as ver", $this->db_Conn);

		if ($arQueryRes = mysql_fetch_array($dbQueryRes, MYSQL_ASSOC)) 
		{
			$curMySqlVer = trim($arQueryRes["ver"]);
			$arCurMySqlVer = explode(".", $curMySqlVer);
			if (IntVal($arCurMySqlVer[0]) > 4 || IntVal($arCurMySqlVer[0]) == 4 && IntVal($arCurMySqlVer[1]) >= 1)
			{
				$bNeedCP = True;
			}
		}
		...
		{
			$mysql_code_page = "";
			$dbQueryRes = @mysql_query("SHOW VARIABLES LIKE 'character_set_server'",  $this->db_Conn);
			if ($arQueryRes = mysql_fetch_array($dbQueryRes, MYSQL_ASSOC))
			{
