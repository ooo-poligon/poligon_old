...
if($oRequest->operation == 'simple_register')
{
	...
	else
	{
		...
		$oResponse->text = GetMessage("MAIN_ADM_CONTROLLER_ERR1");
	}
}
...
else
{
	switch($oRequest->operation)
	{
		...
		case "check_auth":
			...
			{
				if($arUser['ACTIVE']=='Y' && md5($arUser['PASSWORD'].'MySalt') == md5(md5($oRequest->arParameters['password']).'MySalt'))
				{
					$arFields = Array(
							"ID",
							"LOGIN",
...
else
{
	...
	else
	{
		...
		if(strlen($_SERVER['HTTP_REFERER'])>0)
			echo '<br>'.'<a href="'.urlencode($_SERVER['HTTP_REFERER']).'">'.GetMessage("MAIN_ADM_CONTROLLER_BACK_URL").'</a>';
	}
