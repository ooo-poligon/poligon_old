...
class CAdminPage
{
	...
	function ShowPopupCSS()
	{
		...
		//$arCSS[] = ADMIN_THEMES_PATH.'/'.ADMIN_THEME_ID.'/jspopup.css'.($bOpera? '':'?'.filemtime($_SERVER["DOCUMENT_ROOT"].ADMIN_THEMES_PATH.'/'.ADMIN_THEME_ID.'/jspopup.css'));
		...
		for ($i = 0, $cnt = count($arCSS); $i < $cnt; $i++)
			$s .= 'jsAdminStyle.Load(\''.CUtil::JSEscape($arCSS[$i]).'\');'."\r\n";
...
var phpVars = {
	'ADMIN_THEME_ID': '".CUtil::addslashes(ADMIN_THEME_ID)."',
	'LANGUAGE_ID': '".CUtil::addslashes(LANGUAGE_ID)."',
	'FORMAT_DATE': '".CUtil::addslashes(FORMAT_DATE)."',
	'FORMAT_DATETIME': '".CUtil::addslashes(FORMAT_DATETIME)."',
	'opt_context_ctrl': ".($aUserOpt["context_ctrl"] == "Y"? "true":"false").",
	'cookiePrefix': '".CUtil::addslashes(COption::GetOptionString("main", "cookie_name", "BITRIX_SM"))."',
	'titlePrefix': '".CUtil::addslashes(COption::GetOptionString("main", "site_name", $_SERVER["SERVER_NAME"]))." - ',
	'messHideMenu': '".CUtil::addslashes(GetMessage("admin_lib_hide_menu"))."',
	'messShowMenu': '".CUtil::addslashes(GetMessage("admin_lib_show_menu"))."',
	'messHideButtons': '".CUtil::addslashes(GetMessage("admin_lib_less_buttons"))."',
	'messShowButtons': '".CUtil::addslashes(GetMessage("admin_lib_more_buttons"))."',
	'messFilterInactive': '".CUtil::addslashes(GetMessage("admin_lib_filter_clear"))."',
	'messFilterActive': '".CUtil::addslashes(GetMessage("admin_lib_filter_set"))."',
	'messFilterLess': '".CUtil::addslashes(GetMessage("admin_lib_filter_less"))."',
	'messLoading': '".CUtil::addslashes(GetMessage("admin_lib_loading"))."',
	'messMenuLoading': '".CUtil::addslashes(GetMessage("admin_lib_menu_loading"))."',
	'messMenuLoadingTitle': '".CUtil::addslashes(GetMessage("admin_lib_loading_title"))."',
	'messNoData': '".CUtil::addslashes(GetMessage("admin_lib_no_data"))."',
	'messExpandTabs': '".CUtil::addslashes(GetMessage("admin_lib_expand_tabs"))."',
	'messCollapseTabs': '".CUtil::addslashes(GetMessage("admin_lib_collapse_tabs"))."',
	'messPanelFixOn': '".CUtil::addslashes(GetMessage("admin_lib_panel_fix_on"))."',
	'messPanelFixOff': '".CUtil::addslashes(GetMessage("admin_lib_panel_fix_off"))."',
	'messPanelCollapse': '".CUtil::addslashes(GetMessage("admin_lib_panel_hide"))."',
	'messPanelExpand': '".CUtil::addslashes(GetMessage("admin_lib_panel_show"))."'
};
</script>
...
class CAdminMenu
{
	...
	function Init($modules)
	{
		...
		usort($aModuleMenu, create_function('$a, $b', 'if($a["sort"] == $b["sort"]) return 0; return ($a["sort"] < $b["sort"])? -1 : 1;'));
...
';
		...
		if($aMenu["dynamic"] == true && !$bSectionActive)
			echo '<td><div class="sign signplus" onclick="JsAdminMenu.ToggleDynSection(this, \''.$aMenu["module_id"].'\', \''.$aMenu["items_id"].'\', '.($level+1).')"></div></td>';
		else
...
';
	...
	function UnEscape($aFilter)
	{
		...
		foreach($aFilter as $flt)
			if(is_string($GLOBALS[$flt]) && strpos($GLOBALS[$flt], "%u") !== false)
				$GLOBALS[$flt] = $GLOBALS["APPLICATION"]->UnJSEscape($GLOBALS[$flt]);
	}
}
...
<input type="button" value="'.GetMessage("admin_lib_edit_cancel").'" name="cancel" onclick="jsPopup.CloseDialog()" title="'.GetMessage("admin_lib_edit_cancel_title").'" />';
			}
			else
			{
				echo '
...
';
	...
	function ShowWarnings($form, $messages, $aFields=false)
	{
		...
		foreach($aMess as $msg)
		{
			...
			if(empty($field_name))
				continue;
			$s .= ($s <> ""? ", ":"")."{'name':'".CUtil::addslashes($field_name)."', 'title':'".CUtil::addslashes(htmlspecialcharsback($msg["text"]))."'}";
		}
		echo '
<script>
'.$this->name.'.ShowWarnings("'.CUtil::addslashes($form).'", ['.$s.']);
</script>
';
	}
}
...
';
		foreach($aAllCols as $header)
			echo '<option value="'.htmlspecialchars($header["id"]).'">'.htmlspecialchars(($header["name"]<>""? $header["name"]:$header["content"])).'</option>';
...
';
		...
		foreach($aAllCols as $header)
			if($header["sort"] <> "")
				echo '<option value="'.htmlspecialchars($header["sort"]).'"'.($by == strtoupper($header["sort"])? ' selected':'').'>'.htmlspecialchars(($header["name"]<>""? $header["name"]:$header["content"])).'</option>';
...
';
	...
	function AddAdminContextMenu($aContext=array(), $bShowExcel=true, $bShowSettings=true)
	{
		...
		if($bShowSettings)
		{
			...
			$aContext[] = array(
				...
				"LINK"=>"javascript:".$this->table_id.".ShowSettings('".htmlspecialchars(CUtil::addslashes($link))."')",
				"ICON"=>"btn_settings",
			);
		}
		if($bShowExcel)
	...
	function IsUpdated($ID)
	{
		...
		//AddMessage2Log("f_".print_r($f, true)."\r\nf_old=".print_r($f_old, true));
	...
	function ActionPost($url = false)
	{
		...
		return $res.'document.getElementById(\'form_'.$this->table_id.'\').onsubmit(); document.getElementById(\'form_'.$this->table_id.'\').submit();';
	}
...
try
{
	...
	targetResultDiv.appendChild(frameResultDiv.cloneNode(true));
}
catch(e)
{
...
class CAdminFileDialog
{
	function ShowScript($arConfig)
	{
		...
		if(!is_dir($_SERVER["DOCUMENT_ROOT"].$arConfig['path']))
			$arConfig['path'] = substr($arConfig['path'], 0, (int) strrpos($arConfig['path'],'/'));
		if ($arConfig['path'] == '/')
			$arConfig['path'] = '';
		...
		if (strlen($functionError) <= 0)
		{
			?>
			<script>
			window.<?=$arConfig['event']?> = function()
			{
				<?if(!$GLOBALS['USER']->CanDoOperation('fileman_view_file_structure')):?>
					alert("<?=GetMessage('BX_FD_NO_PERMS')?>");
					return;
				<?else:?>
				...
				var oConfig =
				{
					submitFuncName : "<?=$arConfig['event']?>Result",
					select : "<?=$arConfig['select']?>",
					operation: "<?=$arConfig['operation']?>",
					showUploadTab : <? echo $arConfig['showUploadTab'] ? 'true' : 'false';?>,
					showAddToMenuTab : <? echo $arConfig['showAddToMenuTab'] ? 'true' : 'false';?>,
					site : "<?=$arConfig['site']?>",
					path : "<?=$arConfig['path']?>",
					lang : "<?=$arConfig['lang']?>",
					fileFilter : "<?=$arConfig['fileFilter']?>",
					allowAllFiles : <? echo $arConfig['allowAllFiles'] ? 'true' : 'false';?>,
					SaveConfig : <? echo $arConfig['SaveConfig'] ? 'true' : 'false';?>
				};

				<?
				$fd_config = stripslashes(CUserOptions::GetOption("fileman", "file_dialog_config","N"));
				if ($fd_config == "N" || !$arConfig['SaveConfig'])
				{
				?>
					var UserConfig =
					{
				...
				else
				{
					...
					$path = ($arConfig['path'] == '' && $res[0] == $arConfig['site']) ? $res[1] : $arConfig['path'];
					?>
					var UserConfig =
					{
						site : "<?=$arConfig['site']?>",
			...
			window.<?=$arConfig['event']?>Result = function(filename, path, site,title,menu)
			{
				<?
				if ($resultDest == "FUNCTION")
					echo $arConfig['arResultDest']["FUNCTION_NAME"]."(filename,path,site".($arConfig['operation'] == 'O' ? '' : ',title,menu').");";
				elseif ($resultDest == "FORM")
					echo "document.".$arConfig['arResultDest']["FORM_NAME"].".".$arConfig['arResultDest']["FORM_ELEMENT_NAME"].".value = (path == '' ? '/' : path+'/')+filename;";
				elseif ($resultDest == "ID")
					echo "document.getElementById('".$arConfig['arResultDest']["ELEMENT_ID"]."').value = (path == '' ? '/' : path+'/')+filename;";
				?>
			}
	...
	{
		if(defined("__FILE_DIALOG_SCRIPT"))
			return;
		define("__FILE_DIALOG_SCRIPT", true);
		?>
		<script>
		...
		if(head)
		{
			...
			script.type = "text/javascript";
			script.src = "/bitrix/js/main/file_dialog.js?v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/bitrix/js/main/file_dialog.js')?>";
...
class CJSPopup
{
	...
	*/
	...
	function ShowTitlebar($title = '')
	{
		...
		?>
		<div class="bx-popup-title"><table cellspacing="0" class="bx-width100">
		<tr>
	...
	function StartButtons($additional = '')
	{
		...
		echo '<div class="bx-popup-buttons" '.$additional.'>'."\r\n";
	}
...
class CAdminForm extends CAdminTabControl
{
	...
	function CAdminForm($name, $tabs, $bCanExpand = true)
	{
		...
		//echo "<pre>",htmlspecialchars(print_r($this->arCustomLabels, true)),"</pre><hr>\n";
		ob_start();
	}
...
<table cellspacing="0">
	...
	<tr valign="center">
		...
		<td>
			<input type="button" name="tabs_copy" id="tabs_copy" value="&nbsp; &gt; &nbsp;" title="<?echo GetMessage("admin_lib_sett_tabs_copy")?>" disabled onclick="OnAdd(this.id);">
		</td>
		<td>
	...
	<tr valign="center">
		...
		<td>
			...
			<input type="button" name="fields_delete" id="fields_delete"  value="&nbsp; &lt; &nbsp;" title="<?echo GetMessage("admin_lib_sett_fields_delete")?>" disabled onclick="OnDelete(this.id);">
		</td>
...
//				echo "<pre>",htmlspecialchars(print_r($arCustomTabs, true)),"</pre><hr>\n";
...
//				echo "<pre>",htmlspecialchars(print_r($this->tabs, true)),"</pre><hr>\n";
			}
		}
		...
		echo '<form method="POST" Action="'.$action.'"  ENCTYPE="multipart/form-data" name="'.$this->name.'_form">';
		parent::Begin();
		while($this->tabIndex < count($this->tabs))
		{
			$this->BeginNextTab();
	...
	function GetCustomLabelHTML($id = false, $content = "")
	{
		...
		elseif(array_key_exists($id, $this->arCustomLabels))
			return ($this->arFields[$id]["required"]? '<span class="required">*</span>': '').htmlspecialcharsex($this->arCustomLabels[$id]);
		else
	...
	function ShowWarnings($form, $messages, $aFields=false)
	{
		parent::ShowWarnings($this->name.'_name', $messages, $aFields);
	}
	...
	function AddViewField($id, $content, $required = false, $html = false)
	{
		$this->tabs[$this->tabIndex]["FIELDS"][$id] = array(
			"id" => $id,
			"required" => $required,
			...
			"html" => (
				$html === false?
				'<td width="40%">'.$this->GetCustomLabelHTML($id, $content).'</td><td>'.htmlspecialcharsex($this->arFieldValues[$id]).'</td>':
				'<td width="40%">'.$this->GetCustomLabelHTML($id, $content).'</td><td>'.$html.'</td>'
			),
		);
	}

	function AddDropDownField($id, $content, $required, $arSelect)
	{
	...
	function AddCheckBoxField($id, $content, $required, $value, $checked)
	{
