...
if($USER->IsAuthorized())
{
		...
		if(is_callable(array('CUtil', 'addslashes')))
		{
			$aFav = array(
				array(
					...
					"ACTION"=>"jsUtils.Redirect(arguments, '".CUtil::addslashes(BX_ROOT."/admin/favorite_edit.php?lang=".LANG."&amp;name=".urlencode($APPLICATION->GetTitle()))."&amp;addurl='+jsUtils.urlencode(document.getElementById('navchain-link').getAttribute('href', 2)));"
				),
				array(
			...
			if(is_callable(array('CAllFavorites','Delete')))
			{
	...
	//Help
	$page = (defined("HELP_FILE") && strpos(HELP_FILE, '/') === false? HELP_FILE : basename($APPLICATION->GetCurPage())).".html";
	$module = (defined("ADMIN_MODULE_NAME")? ADMIN_MODULE_NAME:"main");
	$help_link = BX_ROOT."/help/".LANGUAGE_ID."/source/".$module."/help/".LANGUAGE_ID."/".$page;
	if(!file_exists($_SERVER["DOCUMENT_ROOT"].$help_link))
	{
		if(file_exists($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/help/".LANGUAGE_ID."/source/main/help/".LANGUAGE_ID."/index.php.html"))
		{
			$help_link = BX_ROOT."/help/".LANGUAGE_ID."/source/main/help/".LANGUAGE_ID."/index.php.html";
		}
		else
		{
			//new help
			$aActiveSection = $adminMenu->ActiveSection();
			$help_link = BX_ROOT."/help/".LANGUAGE_ID."/source/".$aActiveSection["help_section"]."/".(defined("HELP_FILE") && strpos(HELP_FILE, '/') !== false?  HELP_FILE.'.html' : $module."/".$page);
			if(!file_exists($_SERVER["DOCUMENT_ROOT"].$help_link))
			{
				if(file_exists($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/help/".LANGUAGE_ID."/source/index.php.html"))
					$help_link = BX_ROOT."/help/".LANGUAGE_ID."/source/index.php.html";
				elseif(LANGUAGE_ID=="ru")
					$help_link = "http://www.bitrixsoft.ru/ru/help/index.html?page=".urlencode("source/".$module."/help/".LANGUAGE_ID."/".$page);
				else
					$help_link = "http://www.bitrixsoft.com/help/index.html?page=".urlencode("source/".$module."/help/".LANGUAGE_ID."/".$page);
			}
		}
	}
	$arPanelButtons[] = array(
	...
	if($bCanViewSettings)
	{
		...
		$arPanelButtons[] = array(
			"TITLE"=>GetMessage("top_panel_update"),
			"LINK"=>"/bitrix/admin/sysupdate.php?lang=".LANGUAGE_ID,
			"ICON"=>"top_panel_update",
		);
	}
...
foreach($arPanelButtons as $item)
{
	...
	{
		if($item["LINK"] <> "")
			echo '<td><a href="'.$item["LINK"].'" hidefocus="true" title="'.$item["TITLE"].'" '.$item["LINK_PARAM"].' class="context-button'.($item["SELECTED"] == true? ' pressed':'').(!empty($item["ICON"])? ' icon'.($item["TEXT"] == ""? ' icon-only':'').'" id="'.$item["ICON"].'"':'"').'>'.$item["TEXT"].'</a></td>';
		elseif(is_array($item["MENU"]) && !empty($item["MENU"]))
		{
			...
			echo '<td><a href="javascript:void(0)" hidefocus="true" onClick="'.$sMenuUrl.'" title="'.$item["TITLE"].'" class="context-button'.($item["SELECTED"] == true? ' pressed':'').(!empty($item["ICON"])? ' icon'.($item["TEXT"] == ""? ' icon-only':'').'" id="'.$item["ICON"].'"':'"').'">'.$item["TEXT"].'<img src="/bitrix/themes/'.ADMIN_THEME_ID.'/images/panel/arr_down.gif" class="arrow" alt=""></a></td>';
		}
		else
			echo '<td><div title="'.$item["TITLE"].'" class="context-text'.(!empty($item["ICON"])? ' icon" id="'.$item["ICON"].'"':'"').'>'.$item["TEXT"].'</div></td>';
	}
}
