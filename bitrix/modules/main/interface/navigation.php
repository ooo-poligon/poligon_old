...
while($NavRecordGroup <= $this->nEndPage):
	if($NavRecordGroup == $this->NavPageNomer)
		...
		echo('&nbsp;<a href="javascript:'.$this->table_id.'.'.$nav_func_name.'(\''.$sUrlPath.'?PAGEN_'.$this->NavNum.'='.$NavRecordGroup.'&amp;SIZEN_'.$this->NavNum.'='.$this->NavPageSize.$strNavQueryString.'\');">'.$NavRecordGroup.'</a>&nbsp;');
	$NavRecordGroup++;
endwhile;
?></td>
<?if($this->NavPageNomer < $this->NavPageCount):?>
		<td><a href="javascript:<?echo $this->table_id?>.<?=$nav_func_name?>('<?echo $sUrlPath.'?PAGEN_'.$this->NavNum.'='.($this->NavPageNomer+1).'&amp;SIZEN_'.$this->NavNum.'='.$this->NavPageSize.$strNavQueryString;?>');"><img src="/bitrix/themes/<?echo ADMIN_THEME_ID?>/images/nav/next.gif" class="navnext" alt="<?echo GetMessage("navigation_next")?>" title="<?echo GetMessage("navigation_next")?>" border="0"></a></td>
		<td><a href="javascript:<?echo $this->table_id?>.<?=$nav_func_name?>('<?echo $sUrlPath.'?PAGEN_'.$this->NavNum.'='.$this->NavPageCount.'&amp;SIZEN_'.$this->NavNum.'='.$this->NavPageSize.$strNavQueryString;?>');"><img src="/bitrix/themes/<?echo ADMIN_THEME_ID?>/images/nav/last.gif" class="navlast" alt="<?echo GetMessage("navigation_last")?>" title="<?echo GetMessage("navigation_last")?>" border="0"></a></td>
<?else:?>
...
?>
		...
		<select name="" onChange="
			...
			if(val == '0')
				<?echo $this->table_id?>.<?=$nav_func_name?>('<?echo $sUrlPath."?PAGEN_".$this->NavNum."=1&amp;SHOWALL_".$this->NavNum."=1".$strNavQueryString;?>');
			else
				<?echo $this->table_id?>.<?=$nav_func_name?>('<?echo $sUrlPath."?PAGEN_".$this->NavNum."=1&amp;SHOWALL_".$this->NavNum."=0"."&amp;SIZEN_".$this->NavNum."="?>'+val+'<?echo $strNavQueryString;?>');
		">
<?
...
if($_REQUEST["SIZEN_".$this->NavNum] > 0 && !in_array($_REQUEST["SIZEN_".$this->NavNum], $aSizes))
	array_unshift($aSizes, $_REQUEST["SIZEN_".$this->NavNum]);
