	...
	)
{
	...
	while ($ar=$a->Fetch())
	{
		...
		if (is_array($arr))	$sel = (in_array($reference_id,$arr)) ? "selected" : "";
	...
	)
{
	...
	if (!is_array($reference_id)) $reference_id = $a["reference_id"];
	if (!is_array($reference)) $reference = $a["reference"];
	...
	while (list($key,$value)=each($reference_id))
	{
		if (is_array($arr))	$sel = (in_array($value,$arr)) ? "selected" : "";
		$strReturnBox = $strReturnBox."<option ".$sel[$key];
		$strReturnBox = $strReturnBox." value=\"".htmlspecialchars($value)."\" ".$sel.">". htmlspecialchars($reference[$key])."</option>";
 	}
	$strReturnBox = $strReturnBox."</select>";
	return $strReturnBox;
}
	...
	)
{
	...
	else $strReturnBox .= "<select $field1 name='$strBoxName' id='$strBoxName' size='1'>";
...
function ParseDateTime($datetime, $format=false)
{
	...
	if (is_array($fm))
	{
		...
		if (is_array($dt))
		{
			foreach($dt as $v) if (strlen(trim($v))>0) $dt_args[] = $v;
			foreach($fm as $v) if (strlen(trim($v))>0) $fm_args[] = $v;
			if (is_array($fm_args) && is_array($dt_args))
			{
				for($i=0; $i<=count($fm_args)-1; $i++)
				{
					$arrResult[$fm_args[$i]] = sprintf("%0".strlen($fm_args[$i])."d", intval($dt_args[$i]));
				}
				return $arrResult;
			}
		}
...
function GetScriptFileExt()
{
	...
	$script_files = COption::GetOptionString("fileman", "~script_files", "php,phtml,php3,pl,php4,php5,cgi,shtml,ico");
	$arScriptFiles = array();
...
function RemoveScriptExtension($name)
{
	...
	foreach(GetScriptFileExt() as $ext)
	{
		...
		while(($p = strpos($check_name, ".".$ext.".")) !== false)
		{
			...
			$name = substr($name, 0, $p).substr($name, $p+strlen($ext)+1);
		}
	}
	return $name;
}
