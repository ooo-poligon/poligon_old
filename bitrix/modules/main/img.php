...
{
	global $arrSaveColor;
	//echo "<pre>"; print_r($arrSaveColor); echo "</pre>";
...
function GetArrayY($arrY, &$MinY, &$MaxY, $max_grid=15, $first_null="Y")
{
	$arrayY = array();
	...
	else
	{
		...
		if ($shiftY<=0) $shiftY =  round(($MaxY-$MinY)/$max_grid,3);
		$i = $MinY;
		if ($shiftY>0)
		{
...
*/
	...
	$PointsX = sizeof($arrayX);	// количество делений по оси X
...
*/
	...
	while ($i<=$PointsX)
	{
		...
		ImageLine($ImageHandle, ceil($xP0), ceil($yP0), ceil($xP1), ceil($yP1),  IMG_COLOR_STYLED);
		$captionX = $arrayX[$i]; // подписи по оси X
		$xCaption = $xP0 - strlen($captionX)*$k[$FontWidth]; // координата X для подписи
		...
		if (is_array($arrTTF_FONT["X"]) && function_exists("ImageTTFText"))
		{
			...
			$captionX = iso2uni(convert_cyr_string($captionX, "w", "i"));
	...
	while ($i<=$PointsY)
	{
		...
		if ($yM1>0 && $yM0>0)
		{
			...
			if (is_array($arrTTF_FONT["Y"]) && function_exists("ImageTTFText"))
			{
				...
				$captionY = iso2uni(convert_cyr_string($captionY, "w", "i"));
