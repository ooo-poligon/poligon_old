...
function GetFileTemplates($lang = LANG, $arTemplates = Array())
{
	...
	for($i=0; $i<count($arDirs); $i++)
	{
		$dir = $arDirs[$i];
		$TEMPLATE = Array();
		if(file_exists($DOCUMENT_ROOT.BX_PERSONAL_ROOT.$dir))
		{
			@include($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT.$dir."/.content.php");
			...
			if($handle = @opendir($DOCUMENT_ROOT.BX_PERSONAL_ROOT.$dir))
			{
				...
				{
					if($file[0] == ".") continue;
					if($APPLICATION->GetFileAccessPermission(BX_PERSONAL_ROOT.$dir."/".$file) < "R") continue;
...
function ParsePath($path, $bLast=false, $url=false, $param="", $bLogical = false)
{
	...
	for($i=0; $i<count($arDirPath); $i++)
	{
		...
		{
			if(!file_exists($DOC_ROOT.$full_path."/".$file."/.section.php"))
				continue;

			@include($DOC_ROOT.$full_path."/".$file."/.section.php");
	...
	while(false !== ($file = @readdir($handle)))
	{
		...
		if($bLogical)
		{
			...
			else
			{
				...
				if(!preg_match('/^\.(.*)?\.menu\.(php|html|php3|php4|php5|phtml)$/', $file, $regs))
				{
...
function SetPrologTitle($prolog, $title)
{
	...
	else
	{
		...
		{
			if(strlen($title)<=0)
				$prolog = eregi_replace("<title>[^<]*</title>", "", $prolog);
			elseif(eregi("<title>[^<]*</title>", $prolog))
				$prolog = eregi_replace("<title>[^<]*</title>", "<title>".$title."</title>", $prolog);
			else
