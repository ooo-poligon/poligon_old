...
{
	if ($session) $FILTER = $_SESSION["SESS_ADMIN"][$varName];
	if(!is_array($FILTER))
		$FILTER = Array();
	global $$FilterLogic;
	if ($action=="set")
...
function ShowFilterLogicHelp()
{
	...
	if ($LogicHelp!="Y")
	{
		...
		function LogicHelp() { window.open('/bitrix/help/".LANGUAGE_ID."/source/general/filter.php.html', '','scrollbars=yes,resizable=yes,width=780,height=500,top='+Math.floor((screen.height - 500)/2-14)+',left='+Math.floor((screen.width - 780)/2-5)); }
		</script>";
	}
...
function GetFilterSqlSearch($arSqlSearch=array(), $FilterLogic="FILTER_logic")
{
	...
	if ($$FilterLogic=="or") $strSqlSearch = "1=2"; else $strSqlSearch = "1=1";
...
function SortingEx($By, $Path = false, $sByVar="by", $sOrderVar="order", $Anchor="nav_start")
{
	...
	$Path = eregi_replace("([?&])".$sByVar."=[^&]*[&]*", "\\1", $Path);
	$Path = eregi_replace("([?&])".$sOrderVar."=[^&]*[&]*", "\\1", $Path);
