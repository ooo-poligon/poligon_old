...
class CPostingTemplate
{
	...
	function GetByID($path="")
	{
		...
		if($path_parts["dirname"] != POSTING_TEMPLATE_DIR)
			return false;
		$arTemplate = array();
	...
	function Execute()
	{
		...
		while(($arRubric=$rubrics->Fetch()) && $time_of_exec===false)
		{
			...
			$sdate = mktime(0, 0, 0, $arSDate[4]+1, $arSDate[3], $arSDate[5]);
	...
	function AddPosting($arRubric)
	{
		...
		$strSql = "UPDATE b_list_rubric SET LAST_EXECUTED=".$DB->CharToDateFunction($arRubric["END_TIME"])." WHERE ID=".$arRubric["ID"];
