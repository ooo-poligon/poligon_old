...
$aTabs = array(
	array("DIV" => "edit1", "TAB" => GetMessage("post_subscribers"), "ICON"=>"main_user_edit", "TITLE"=>GetMessage("post_tab_title")),
);
...
if(is_array($GROUP_ID) && count($GROUP_ID)>0)
{
	$user = CUser::GetList(
		($b="id"), ($o="asc"), 
		array("GROUP_MULTI"=>$GROUP_ID, "ACTIVE"=>"Y", "EMAIL"=>$EMAIL_FILTER)
	);
...
function SetValues()
{
	...
	d.getElementById('EMAIL_FILTER').value="<?echo $EMAIL_FILTER?>";
	d.getElementById('SUBSCR_FORMAT').value="<?echo $SUBSCR_FORMAT?>";
	<?foreach($aRub as $id):?>
