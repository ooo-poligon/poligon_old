...
?>
	...
	<tr>
		...
		{
			$user = CUser::GetByID($str_USER_ID);
			if($user->ExtractFields("user_"))
				$sUser = "[<a href=\"user_edit.php?ID=".$str_USER_ID."&amp;lang=".LANG."\">".$str_USER_ID."</a>] (".$user_LOGIN.") ".$user_NAME." ".$user_LAST_NAME;
		}
...
?>
	...
	<tr>
		...
		<?
			...
			if(!is_array($RUB_ID)) $RUB_ID = array();
			$rub = CRubric::GetList(array("LID"=>"ASC", "SORT"=>"ASC", "NAME"=>"ASC"), array("ACTIVE"=>"Y"));
			$n=1;
			while($rub->ExtractFields("r_")):?>
				<input type="checkbox" id="RUB_ID_<?echo $n?>" name="RUB_ID[]" value="<?echo $r_ID?>"<?if(in_array($r_ID, ($bVarsFromForm? $RUB_ID:$aSubscrRub))) echo " checked"?>>
				<label for="RUB_ID_<?echo $n?>"><?echo "[".$r_LID."] ".$r_NAME?></label><br>
			<? 
				$n++;
			endwhile;
		?>
