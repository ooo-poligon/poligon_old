...
<?if($ID>0 && !$bCopy):?>
	...
	<?endif;?>
	<?if($str_SENT_BCC <> "" || $str_ERROR_EMAIL <> ""):?>
	<tr>
		<td><?echo GetMessage("POST_TO")?></td>
		<td>[&nbsp;<a class="tablebodylink" href="javascript:void(0)" OnClick="jsUtils.OpenWindow('posting_bcc.php?ID=<?echo $str_ID?>&lang=<?echo LANG?>', 600, 500);"><?echo GetMessage("POST_SHOW_LIST")?></a>&nbsp;]</td>
	</tr>
	<?endif;?>
<?endif; //ID?>
	<tr>
...
?>
	<?if($ID>0 && ($post_file = CPosting::GetFileList($ID)) && $post_file->ExtractFields("f_")):?>
	<tr>
...
?>
			<tr>
				<td><?echo $f_ORIGINAL_NAME?></td>
				<td align="right"><?echo $f_FILE_SIZE?></td>
				<td align="center"><input type="checkbox"  name="FILE_ID[]" value="<?echo $f_ID?>"></td>
			</tr>
<?
		} while($post_file->ExtractFields("f_"));
?>
		</table></td>
	</tr>
	<?endif;?>
	<tr>
		...
		<td>
			...
			<tr><td><?echo CFile::InputFile("NEW_FILE[]", 40, 0)?></td></tr>
			<tr><td><?echo CFile::InputFile("NEW_FILE[]", 40, 0)?></td></tr>
			<tr><td><?echo CFile::InputFile("NEW_FILE[]", 40, 0)?></td></tr>
			</table>
		</td>
...
?>
	...
	<tr>
		...
		<?
		$aCharset = split(",", COption::GetOptionString("subscribe", "posting_charset"));
		for($i=0; $i<count($aCharset); $i++):
			?><option value="<?echo $aCharset[$i]?>"<?if($str_CHARSET==$aCharset[$i]) echo" selected"?>><?echo $aCharset[$i]?></option><?
		endfor;
		?>
		</select>
		</td>
