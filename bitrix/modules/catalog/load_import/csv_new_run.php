...
$NUM_CATALOG_LEVELS = IntVal(COption::GetOptionString("catalog", "num_catalog_levels", 4));
				...
				for ($i = 0; $i < $NUM_FIELDS; $i++)
				{
					if (substr(${"field_".$i}, 0, 7) == "IP_PROP")
					{
						...
						if (is_array($arIBlockProperty[$cur_prop_id]) && count($arIBlockProperty[$cur_prop_id])>0)
						{
							...
							{
								if (!isset($arIBlockPropertyValue[$PRODUCT_ID][$cur_prop_id])
							...
							/**
							* костыль дл€ схемы импорта "ћайские тезисы"
							* дополнительного пол€ содержащего текст
							* рецепт вз€т отсюда http://dev.1c-bitrix.ru/community/forums/forum6/topic37613/
							* добавлена 1 строки
							*/
							if ($cur_prop_id=="17") { $PROP[17] = array('VALUE'=>array('TYPE'=>'HTML', 'TEXT'=>$arRes[$i])); }
							/**
							* дополнительный html текст, псоле таблицы тех. характеристик
							* ниже ссылки на pdf
							*/
							if ($cur_prop_id=="74") { $PROP[74] = array('VALUE'=>array('TYPE'=>'HTML', 'TEXT'=>$arRes[$i])); }
