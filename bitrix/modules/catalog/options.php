...
if ($REQUEST_METHOD=="GET" && strlen($RestoreDefaults)>0 && !$bReadOnly /*$CAT_RIGHT=="W"*/)
{
	if (!$USER->IsAdmin())
...
$arAllOptions =
	Array(
		Array("export_default_path", GetMessage("CAT_EXPORT_DEFAULT_PATH"), "/upload/", Array("text", 30)),
...
$strWarning = "";
$strOK = "";
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && !$bReadOnly /*$CAT_RIGHT=="W"*/)
{
	...
	if ($USER->IsAdmin())
	{
		...
		if (strlen($oldAvailContentGroups) > 0)
			$arOldAvailContentGroups = split(",", $oldAvailContentGroups);
