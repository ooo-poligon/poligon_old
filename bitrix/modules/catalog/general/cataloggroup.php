...
class CAllCatalogGroup
{
	...
	function GetListArray()
	{
		...
		else
		{
			...
			global $CACHE_MANAGER;
			if ($CACHE_MANAGER->Read($cacheTime, "catalog_group_".LANGUAGE_ID))
			{
	...
	function Update($ID, $arFields)
	{
		...
		if (!defined("CATALOG_SKIP_CACHE") || !CATALOG_SKIP_CACHE)
		{
			$dbLang = CLang::GetList($by = 'sort', $order = 'asc');
			while ($arLang = $dbLang->Fetch())
				$GLOBALS["CACHE_MANAGER"]->Clean("catalog_group_".$arLang['LID']);
	...
	function Delete($ID)
	{
		...
		if ($res = CCatalogGroup::GetByID($ID))
		{
			if ($res["BASE"] != "Y")
			{
				...
				if (!defined("CATALOG_SKIP_CACHE") || !CATALOG_SKIP_CACHE)
				{
					$GLOBALS["CACHE_MANAGER"]->Clean("catalog_group");
					$GLOBALS["CACHE_MANAGER"]->Clean("catalog_group_perms");
				}
