...
class CAllCatalogProduct
{
	...
	function GetByIDEx($ID)
	{
		...
		{
			if ($arIBlock = GetIBlock($arIBlockElement["IBLOCK_ID"]))
			{
	...
	function GetByID($ID)
	{
		...
		if (isset($GLOBALS["CATALOG_PRODUCT_CACHE_".$ID]) && is_array($GLOBALS["CATALOG_PRODUCT_CACHE_".$ID]) && isset($GLOBALS["CATALOG_PRODUCT_CACHE_".$ID]["ID"]))
		{
			return $GLOBALS["CATALOG_PRODUCT_CACHE_".$ID];
		}
		else
		{
			$strSql = 
			...
			if ($res = $db_res->Fetch())
			{
				$GLOBALS["CATALOG_PRODUCT_CACHE_".$ID] = $res;
				return $res;
			}
		}
	...
	// Изменяет отслеживаемое количество товара $PRODUCT_ID на $DELTA_QUANTITY, иначе false
	...
	function Update($ID, $arFields)
	{
		...
		if (StrLen($strUpdate) > 0)
		{
			...
			unset($GLOBALS["CATALOG_PRODUCT_CACHE_".$ID]);
		}
