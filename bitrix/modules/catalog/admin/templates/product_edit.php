...
function checkForm(e)
{
	...
	{
		alert('<?=CUtil::JSEscape(GetMessage("IBEL_E_BASE_PRICE"))?>');
		bReturn = false;
	}
	else
	{
...
jsUtils.addEvent(getElementForm(), 'submit', checkForm);
		...
		<table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table" id="catalog_price_table">
			<tr>
				...
				<td width="60%">
					...
					?>
					<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> id="CAT_BASE_PRICE" name="CAT_BASE_PRICE" value="<?echo htmlspecialchars($str_CAT_BASE_PRICE) ?>" size="30" OnChange="ChangeBasePrice()">
				</td>
			</tr>
			<tr>
				<td>
...
<div id="prices_ext" style="display: <?=$bUseExtendedPrice ? 'block' : 'none'?>;">
		...
		function CloneBasePriceGroup()
		{
			...
			oCell.align = "center";
			//oCell.className = "tablebody";
			oCell.innerHTML = '<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> id="CAT_BASE_PRICE_'+cnt+'" name="CAT_BASE_PRICE_'+cnt+'" value="" size="15" OnChange="ChangeBasePriceEx(this)">';
			...
			{
				?>str += '<option value="<?echo $arCurrencyList["CURRENCY"] ?>"><?echo $arCurrencyList["CURRENCY"]?> (<?echo htmlspecialchars($arCurrencyList["FULL_NAME"])?>)</option>';<?
			}
			?>
			str += '</select>';
			oCell.innerHTML = str;
						...
						<tbody id="container3">
							...
							for ($i = 0; $i < count($arPriceBoundaries); $i++)
							{
								...
								<tr id="model3">
									...
									<td valign="top" align="center">
										<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> id="CAT_BASE_PRICE_<?= $ind ?>" name="CAT_BASE_PRICE_<?= $ind ?>" value="<?echo htmlspecialchars($str_CAT_BASE_PRICE) ?>" size="15" OnChange="ChangeBasePriceEx(this)">
									</td>
									<td valign="top" align="center">
										<?$db_curr = CCurrency::GetList(($by1="sort"), ($order1="asc"));?>
							...
							if ($ind == -1)
							{
								...
								<tr id="model3">
									...
									<td valign="top" align="center">
										<input type="text" <?if ($bReadOnly) echo "disabled readonly" ?> id="CAT_BASE_PRICE_<?= $ind ?>" name="CAT_BASE_PRICE_<?= $ind ?>" value="" size="15" OnChange="ChangeBasePriceEx(this)">
									</td>
									<td valign="top" align="center">
										<?$db_curr = CCurrency::GetList(($by1="sort"), ($order1="asc"));?>
										<select id="CAT_BASE_CURRENCY_<?= $ind ?>" name="CAT_BASE_CURRENCY_<?= $ind ?>" <?if ($bReadOnly) echo "disabled readonly" ?> OnChange="ChangeBaseCurrencyEx(this)">
...
SetFieldsStyle('catalog_properties_table');
		...
		$dbProductDiscounts = CCatalogDiscount::GetList(
				...
				array(
						"+PRODUCT_ID" => $PRODUCT_ID,
						"+SECTION_ID" => $str_IBLOCK_ELEMENT_SECTION,
						"ACTIVE" => "Y",
				...
				false,
				array(
						"ID", "SITE_ID", "ACTIVE", "ACTIVE_FROM", "ACTIVE_TO", "RENEWAL", "NAME", "SORT", "MAX_DISCOUNT", "VALUE_TYPE", "VALUE", "CURRENCY", "PRODUCT_ID"
					)
			);
			...
			<table border="0" cellspacing="0" cellpadding="0" class="internal" align="center" width="100%">
				...
				do
				{
					...
					<tr>
						...
						<td align="left">
							<a href="cat_discount_edit.php?ID=<?= $arProductDiscounts["ID"] ?>&lang=<?= LANG ?>#tb" target="_blank"><?echo GetMessage("C2IT_MODIFY")?></a>
						</td>
					</tr>
					<?
				}
