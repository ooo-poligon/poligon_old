...
include(GetLangFileName($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/catalog/lang/", "/data_import.php"));
...
if ($max_execution_time<=0) $max_execution_time = 0;
...
if (($REQUEST_METHOD == "POST" || $CUR_FILE_POS>0) && $STEP > 1 /*&& $CATALOG_RIGHT=="W"*/)
{
	...
	if ($STEP>1)
	{
		...
		if (strlen($strError)<=0)
		{
			...
			if (CCatalog::GetByID($IBLOCK_ID)) $bIBlockIsCatalog = True;
			...
			if ($arRes)
			{
				...
				do
				{
					...
					if (strlen($strErrorR)<=0 && $bIBlockIsCatalog)
					{
						...
						{
							if (substr(${"field_".$j}, 0, 9) == "CR_PRICE_")
							{
								$sPRICE_ID = substr(${"field_".$j}, 9);
								$arSPRICE_ID = Split("_", $sPRICE_ID);
								$PRICE_ID = IntVal($arSPRICE_ID[0]);
								$OFFER_CURRENCY = $arSPRICE_ID[1];
								$OFFER_PRICE = str_replace(",", ".", Trim($arRes[$j]));
								$OFFER_PRICE = preg_replace("/[^\d.]/i", "", $OFFER_PRICE);
								$OFFER_PRICE = DoubleVal($OFFER_PRICE);
								...
								$res = CPrice::GetList($o, $b, Array("PRODUCT_ID"=>$PRODUCT_ID, "CATALOG_GROUP_ID"=>$PRICE_ID));
								if($arr = $res->Fetch())
								{
		...
		if ($FM_RIGHT!="D"):
			...
			<tr>
				<td align="right" nowrap class="tablebody" valign="top">
					<font class="tablefieldtext"><?echo GetMessage("CATI_DATA_FILE_SITE") ?><br><?echo GetMessage("CATI_FROM_DISK") ?></font>
				</td>
				<td align="left" nowrap class="tablebody">
					<?
					$path = Rel2Abs("/", $path);
