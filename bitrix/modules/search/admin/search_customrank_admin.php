...
while($arRes = $rsData->NavNext(true, "f_")):
	...
	$strPARAM1=
		'<select name="FIELDS['.$f_ID.'][PARAM1]">'.
	...
	$f_PARAM1_NAME="[".$f_PARAM1."] ".htmlspecialchars(CSearchCustomRank::__GetParam($lang, $f_SITE_ID, $f_MODULE_ID, $f_PARAM1));
	$row->AddViewField("PARAM1",($f_PARAM1==""?"&nbsp;":$f_PARAM1_NAME));

	$strPARAM2=
		'<select name="FIELDS['.$f_ID.'][PARAM2]" OnChange="BoxUpdateNew(\'param1\', '.$f_ID.')">'.
		'<option value="">'.GetMessage("customrank_no").'</option>';
	if($f_MODULE_ID=="iblock" && CModule::IncludeModule("iblock")):
	...
	$f_PARAM2_NAME="[".$f_PARAM2."] ".htmlspecialchars(CSearchCustomRank::__GetParam($lang, $f_SITE_ID, $f_MODULE_ID, $f_PARAM1, $f_PARAM2));
	$row->AddViewField("PARAM2",($f_PARAM2==""?"&nbsp;":$f_PARAM2_NAME));

	$row->AddInputField("ITEM_ID", array("size"=>5));
	$f_ITEM_ID_NAME="[".$f_ITEM_ID."] ".htmlspecialchars(CSearchCustomRank::__GetParam($lang, $f_SITE_ID, $f_MODULE_ID, $f_PARAM1, $f_PARAM2, $f_ITEM_ID));
...
function BoxUpdateParam1(id)
{
	GetAdminDiv('PARAM1['+id+']_result_div'
		...
		+'&'+'PARAM=1'
		+'&'+'ID='+id
		+'&'+'FIELD_ID=FIELDS['+id+'][PARAM1]'
		+'&'+'SITE_ID='+document.getElementsByName('FIELDS['+id+'][SITE_ID]')[0].value
		+'&'+'MODULE_ID='+document.getElementsByName('FIELDS['+id+'][MODULE_ID]')[0].value
		+'&'+'PARAM1='+document.getElementsByName('FIELDS['+id+'][PARAM1]')[0].value
	,true, false);
}
function BoxUpdateParam2(id)
{
	GetAdminDiv('PARAM2['+id+']_result_div'
		...
		+'&'+'PARAM=2'
		+'&'+'ID='+id
		+'&'+'FIELD_ID=FIELDS['+id+'][PARAM2]'
		+'&'+'SITE_ID='+document.getElementsByName('FIELDS['+id+'][SITE_ID]')[0].value
		+'&'+'MODULE_ID='+document.getElementsByName('FIELDS['+id+'][MODULE_ID]')[0].value
		+'&'+'PARAM1='+document.getElementsByName('FIELDS['+id+'][PARAM1]')[0].value
		+'&'+'PARAM2='+document.getElementsByName('FIELDS['+id+'][PARAM2]')[0].value
	,false, true);
}
