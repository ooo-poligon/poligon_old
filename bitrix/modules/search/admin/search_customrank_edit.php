...
$aTabs = array(
	array("DIV" => "edit1", "TAB" => GetMessage("customrank_rule"), "ICON"=>"main_user_edit", "TITLE"=>GetMessage("customrank_rule_title")),
);
...
$APPLICATION->SetTitle(($ID>0?GetMessage("customrank_edit_rule").$ID :GetMessage("customrank_add_rule")));

require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");

$aMenu = array(
	array(
		"TEXT"=>GetMessage("customrank_list_rule"),
		"TITLE"=>GetMessage("customrank_list_rule_title"),
...
if($ID>0)
{
	...
	$aMenu[] = array(
		"TEXT"=>GetMessage("customrank_add_new"),
		"TITLE"=>GetMessage("customrank_add_new_title"),
		"LINK"=>"search_customrank_edit.php?lang=".LANG,
		"ICON"=>"btn_new",
	);
	$aMenu[] = array(
		"TEXT"=>GetMessage("customrank_delete"),
		"TITLE"=>GetMessage("customrank_delete_title"),
		"LINK"=>"javascript:if(confirm('".AddSlashes(GetMessage("customrank_delete_conf"))."'))window.location='search_customrank_admin.php?ID=".$ID."&cf=delid&lang=".LANG."&".bitrix_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
}
...
function ParamUpdate()
{
	...
	{
		case 'main':
			document.getElementById('param1_label').innerHTML='<?=AddSlashes(GetMessage("customrank_param1"))?>';
			document.getElementById('param2_label').innerHTML='<?=AddSlashes(GetMessage("customrank_param2"))?>';
			document.getElementById('item_id_label').innerHTML='<?=AddSlashes(GetMessage("customrank_param3_main"))?>';
			document.getElementById('main_item_id').style.display="block";
			break;
		case 'iblock':
			document.getElementById('param1_label').innerHTML='<?=AddSlashes(GetMessage("customrank_param1_iblock"))?>';
			document.getElementById('param2_label').innerHTML='<?=AddSlashes(GetMessage("customrank_param2_iblock"))?>';
			document.getElementById('item_id_label').innerHTML='<?=AddSlashes(GetMessage("customrank_param3_iblock"))?>';
			document.getElementById('iblock_item_id').style.display="block";
			break;
		case 'forum':
			document.getElementById('param1_label').innerHTML='<?=AddSlashes(GetMessage("customrank_param1_forum"))?>';
			document.getElementById('param2_label').innerHTML='<?=AddSlashes(GetMessage("customrank_param2_forum"))?>';
			document.getElementById('item_id_label').innerHTML='<?=AddSlashes(GetMessage("customrank_param3_forum"))?>';
			document.getElementById('forum_item_id').style.display="block";
			break;
		default:
			document.getElementById('param1_label').innerHTML='<?=AddSlashes(GetMessage("customrank_param1"))?>';
			document.getElementById('param2_label').innerHTML='<?=AddSlashes(GetMessage("customrank_param2"))?>';
			document.getElementById('item_id_label').innerHTML='<?=AddSlashes(GetMessage("customrank_param3"))?>';
			document.getElementById('main_item_id').style.display="block";
			break;
	}
}
...
?>
	<tr>
		<td width="40%"><?=GetMessage("customrank_site")?></td>
	...
	<tr>
		...
		<option value="main"<?=$str_MODULE_ID=="main"?" selected":""?>><?=GetMessage("customrank_files")?></option>
		<option value="iblock"<?=$str_MODULE_ID=="iblock"?" selected":""?>><?=GetMessage("customrank_iblocks")?></option>
		<option value="forum"<?=$str_MODULE_ID=="forum"?" selected":""?>><?=GetMessage("customrank_forum")?></option>
		</select>
		</td>
	</tr>
	<tr>
		<td><span id="param1_label"><?=GetMessage("customrank_param1")?></span></td>
		<td>
		...
		<?if($str_MODULE_ID=="iblock" && CModule::IncludeModule("iblock")):?>
			...
			while($arr=$rsType->Fetch())
			{
				...
				<option value="<?=htmlspecialchars($arr["ID"])?>"<?=$arr["ID"]==$str_PARAM1?" selected":""?>><?=htmlspecialchars("[".$arr["ID"]."] ".$ar["NAME"])?></option>
				<?endif;
			}
		?>
			</select>
	...
	<tr>
		<td><span id="param2_label"><?=GetMessage("customrank_param2")?></span></font></td>
		<td>
	...
	<tr>
		<td><span id="item_id_label"><?=GetMessage("customrank_param3")?></span></td>
		<td>
		...
		<input type="button" value="..." id="ITEM_ID_CHOOSE" onClick="jsUtils.WindowOpen('iblock_element_search.php?lang=<?=LANG?>&amp;IBLOCK_ID='+document.getElementById('PARAM2').value+'&amp;n=ITEM_ID&amp;k=KEY', 600, 500);">
		&nbsp;<span id="sp_<?=md5("ITEM_ID")?>_<?="KEY"?>" class="tablebodytext">
		<?
		if($str_MODULE_ID=="iblock" && $str_PARAM1!="" && $str_PARAM2!="" && $str_ITEM_ID!="")
			echo htmlspecialchars(CSearchCustomRank::__GetParam($lang, $str_SITE_ID, $str_MODULE_ID, $str_PARAM1, $str_PARAM2, $str_ITEM_ID));
		?>
		</span>
		</div>
	...
	<tr>
		<td><?=GetMessage("customrank_sort")?></td>
		<td>
