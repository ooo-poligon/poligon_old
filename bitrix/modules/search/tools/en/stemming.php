...
function stemming_en($word)
{
	...
	$vowels = "AEIOUY";
	$word=ereg_replace("^(Y)","y",$word);
	$word=ereg_replace("([$vowels])(Y)","\\1y",$word);
	...
	$found=array();
	if(ereg("(SSES|IED|IES|US|SS|S)$", $word, $found))
		switch ($found[0]) {
			//sses - replace by ss
			case "SSES":
	...
	if(ereg("(EEDLY|INGLY|EDLY|EED|ING|ED)$", $word, $found))
		switch($found[0]) {
			case "EEDLY":
			case "EED":
				if(ereg($found[0]."$", substr($word, $R1)))
					$word = substr($word, 0, strlen($word)-strlen($found[0]))."EE";
				break;
			default:
				//delete if the preceding word part contains a vowel, and then
				if(($step1b=ereg_replace("([$vowels].*)(ED|EDLY|ING|INGLY)$", "\\1", $word))!=$word)
				{
					//if the word ends at, bl or iz add e (so luxuriat -> luxuriate), or
					if(($step1b1=ereg_replace("(AT|BL|IZ)$", "\\1E", $step1b))==$step1b)
						//if the word ends with a double remove the last letter (so hopp -> hop), or
						if(ereg("(BB|DD|FF|GG|MM|NN|PP|RR|TT)$", $step1b))
					...
					//Define a short syllable in a word as either (a) a vowel followed by a non-vowel other than w, x or Y and preceded by a non-vowel, or * (b) a vowel at the beginning of the word followed by a non-vowel.
							if(	ereg("^[^$vowels]+[$vowels][^WXy$vowels]$", $step1b)
								|| ereg("^[$vowels][^$vowels]$", $step1b))
								$step1b1 = $step1b."E";
						}
					$step1b = $step1b1;
				}
	...
	$word=ereg_replace("^(.+[^$vowels])([yY])$",  "\\1I", $word);
	//Step 2:
	//	Search for the longest among the following suffixes, and, if found and in R1, perform the action indicated.
	if(ereg($STEMMING_EN_STEP2, $word, $found) && ereg($found[0]."$", substr($word, $R1)))
		switch ($found[0]) {
			case "OGI":
				if(ereg("LOGI$", $word))
	...
	//Step 3:
	//	Search for the longest among the following suffixes, and, if found and in R1, perform the action indicated.
	if(ereg($STEMMING_EN_STEP3, $word, $found) && ereg($found[0]."$", substr($word, $R1)))
		switch($found[0]) {
			case "ATIVE":
				if(ereg("ATIVE$", substr($word, $R2)))
	...
	if(ereg($STEMMING_EN_STEP4, $word, $found) && ereg($found[0]."$", substr($word, $R2)))
		switch($found[0]) {
			case "ION":
				if(ereg("[ST]ION$", $word))
	...
	//Step 5:
	if(ereg("E$", substr($word, $R2))
		|| (ereg("E$", substr($word, $R1))
			&& !
			//Define a short syllable in a word as either (a) a vowel followed by a non-vowel other than w, x or Y and preceded by a non-vowel, or * (b) a vowel at the beginning of the word followed by a non-vowel.
					(ereg("[^$vowels][$vowels][^WXy$vowels].$", $word)
						|| ereg("^[$vowels][^$vowels].$", $word))
			))
		$word = substr($word, 0, strlen($word)-1);
	elseif(ereg("L$", substr($word, $R2)) && ereg("LL$", $word))
		$word = substr($word, 0, strlen($word)-1);
