...
IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/iblock/admin/iblock_type_admin.php');
...
$aTabs = array();
$aTabs[] = array("DIV" => "edit1", "TAB" => GetMessage("IBTYPE_E_TAB1"), "ICON"=>"iblock_type", "TITLE"=>GetMessage("IBTYPE_E_TAB1_T"));
$aTabs[] = array("DIV" => "edit2", "TAB" => GetMessage("IBTYPE_E_TAB2"), "ICON"=>"iblock_type", "TITLE"=>GetMessage("IBTYPE_E_TAB2_T"));
...
if($REQUEST_METHOD=="POST" && strlen($Update)>0 && check_bitrix_sessid())
{
	...
	if(!$res)
	{
		$strWarning.= GetMessage("IBLOCK_TYPE_SAVE_ERROR").": ".$obBlocktype->LAST_ERROR."";
		$DB->Rollback();
		$bVarsFromForm = true;
	}
	else
...
$aMenu = array(
	array(
		...
		"TITLE" => GetMessage("IBTYPE_E_LIST"),
...
if(strlen($ID)>0)
{
	...
	$aMenu[] = array(
		...
		"TITLE" => GetMessage("MAIN_NEW_RECORD"),
	...
	$aMenu[] = array(
		...
		"TITLE" =>GetMessage("MAIN_DELETE_RECORD"),
		"LINK"	=> "javascript:if(confirm('".GetMessage("IBLOCK_TYPE_ADMIN_DEL_CONF")."')) window.location='/bitrix/admin/iblock_type_admin.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."';",
		"ICON" => "btn_delete"
		);
}
	...
	<tr>
		<td colspan="2" align="center">
			<table border="0" cellspacing="6">
				<tr>
					<td align="center"><?echo GetMessage("IBLOCK_TYPE_LANG");?></td>
					<td align="center"><span class="required">*</span><?echo GetMessage("IBLOCK_TYPE_NAME");?></td>
					<td align="center"><span id="SECTION_NAME_TITLE"><?echo GetMessage("IBLOCK_TYPE_SECTIONS");?></span></td>
					<td align="center"><?echo GetMessage("IBLOCK_TYPE_ELEMENTS");?></td>
				</tr>
				<?
				for($i=0; $i<count($arIBTLang); $i++):
					if($bVarsFromForm)
...
<?$tabControl->BeginNextTab();?>
	<tr>
		<td width="40%"><?echo GetMessage("IBLOCK_OPTION_USE_RSS")?></td>
