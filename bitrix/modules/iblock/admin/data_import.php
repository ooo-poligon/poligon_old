	...
	if ($STEP > 1)
	{
		...
		if (is_uploaded_file($_FILES["DATA_FILE"]["tmp_name"]))
		{
			if(strtolower(GetFileExtension($_FILES["DATA_FILE"]["name"])) != "csv")
				$strError .= GetMessage("ICI_NOT_CSV")."<br>";
			else
			{
				...
				for ($i = 0; $i < $NUM_CATALOG_LEVELS; $i++)
				{
					...
					if($bOK)
					{
						...
						if(strlen($arGroupsTmp1["NAME"])<=0)
							$arGroupsTmp1["NAME"] = GetMessage("CATI_NOMAME");
				...
				if (strlen($strErrorR)<=0)
				{
					...
					if ($arr = $res->Fetch())
					{
						...
						if($arr["TMP_ID"] != $tmpid)
						{
							...
							$res = $el->Update($PRODUCT_ID, $arLoadProductArray, $bWorkFlow, false);
						}
						else
							$res = true;
					}
					else
					{
						...
						$PRODUCT_ID = $el->Add($arLoadProductArray, $bWorkFlow, false);
						$res = ($PRODUCT_ID>0);
					}
...
if (!$bAllLinesLoaded)
{
	$strParams = bitrix_sessid_get()."&CUR_FILE_POS=".$curFilePos."&CUR_LOAD_SESS_ID=".$CUR_LOAD_SESS_ID."&STEP=4&URL_DATA_FILE=".urlencode($DATA_FILE_NAME)."&IBLOCK_ID=".$IBLOCK_ID."&fields_type=".urlencode($fields_type)."&max_execution_time=".IntVal($max_execution_time);
	if ($fields_type=="R")
...
<?
$aTabs = array(
	array("DIV" => "edit1", "TAB" => GetMessage("ICI_TAB1"), "ICON" => "iblock", "TITLE" => GetMessage("ICI_TAB1_ALT")),
	array("DIV" => "edit2", "TAB" => GetMessage("ICI_TAB2"), "ICON" => "iblock", "TITLE" => GetMessage("ICI_TAB2_ALT")),
	array("DIV" => "edit3", "TAB" => GetMessage("ICI_TAB3"), "ICON" => "iblock", "TITLE" => GetMessage("ICI_TAB3_ALT")),
	array("DIV" => "edit4", "TAB" => GetMessage("ICI_TAB4"), "ICON" => "iblock", "TITLE" => GetMessage("ICI_TAB4_ALT")),
);
...
if ($STEP == 1)
{
	?>
 	<tr>
		<td><?echo GetMessage("CATI_DATA_FILE") ?></td>
		<td>
			<input type="text" name="URL_DATA_FILE" size="30">
			<input type="button" value="<?echo GetMessage("ICI_OPEN") ?>" OnClick="BtnClick()">
			<?
	...
	<tr>
		<td><?echo GetMessage("CATI_INFOBLOCK") ?></td>
		<td>
...
if ($STEP == 2)
{
	...
	<tr>
		<td valign="middle" colspan="2">
			...
			<input type="radio" name="fields_type" id="fields_type_R" value="R" <?if ($fields_type=="R" || strlen($fields_type)<=0) echo "checked";?> onClick="ChangeExtra()"><label for="fields_type_R"><?echo GetMessage("CATI_RAZDELITEL") ?></label><br>
			<input type="radio" name="fields_type" id="fields_type_F" value="F" <?if ($fields_type=="F") echo "checked";?> onClick="ChangeExtra()"><label for="fields_type_F"><?echo GetMessage("CATI_FIXED") ?></label>

		</td>
	</tr>

	<tr id="table_r" class="heading">
		<td colspan="2"><?echo GetMessage("CATI_RAZDEL1") ?></td>
	</tr>
	<tr id="table_r1">
		<td valign="top" width="40%"><?echo GetMessage("CATI_RAZDEL_TYPE") ?></td>
		<td valign="top" width="60%">
			<input type="radio" name="delimiter_r" id="delimiter_r_TZP" value="TZP" <?if ($delimiter_r=="TZP" || strlen($delimiter_r)<=0) echo "checked"?>><label for="delimiter_r_TZP"><?echo GetMessage("CATI_TZP") ?></label><br>
			<input type="radio" name="delimiter_r" id="delimiter_r_ZPT" value="ZPT" <?if ($delimiter_r=="ZPT") echo "checked"?>><label for="delimiter_r_ZPT"><?echo GetMessage("CATI_ZPT") ?></label><br>
			<input type="radio" name="delimiter_r" id="delimiter_r_TAB" value="TAB" <?if ($delimiter_r=="TAB") echo "checked"?>><label for="delimiter_r_TAB"><?echo GetMessage("CATI_TAB") ?></label><br>
			<input type="radio" name="delimiter_r" id="delimiter_r_SPS" value="SPS" <?if ($delimiter_r=="SPS") echo "checked"?>><label for="delimiter_r_SPS"><?echo GetMessage("CATI_SPS") ?></label><br>
			<input type="radio" name="delimiter_r" id="delimiter_r_OTR" value="OTR" <?if ($delimiter_r=="OTR") echo "checked"?>><label for="delimiter_r_OTR"><?echo GetMessage("CATI_OTR") ?></label>
			<input type="text" name="delimiter_other_r" size="3" value="<?echo htmlspecialchars($delimiter_other_r) ?>">
		</td>
	</tr>
	<tr id="table_r2">
		<td><?echo GetMessage("CATI_FIRST_NAMES") ?></td>
		<td>
	...
	<tr id="table_f" class="heading">
		<td colspan="2"><?echo GetMessage("CATI_FIX1") ?></td>
	</tr>
	<tr id="table_f1">
...
if ($STEP == 3)
{
	...
	for ($i = 0; $i < count($arDataFileFields); $i++)
	{
		...
		<tr>
			<td valign="top" width="40%">
				<b><?echo GetMessage("CATI_FIELD") ?> <?echo $i+1 ?></b> (<?echo htmlspecialchars($arDataFileFields[$i]);?>):
			</td>
			<td valign="top" width="60%">
	...
	<tr>
		<td valign="top"><?echo GetMessage("CATI_IMG_PATH") ?>:</td>
		<td valign="top">
	...
	<tr>
		<td valign="top"><?echo GetMessage("CATI_OUTFILE") ?>:</td>
		<td valign="top">
			<input type="radio" id="outFileAction_H" name="outFileAction" value="H" <?if (strlen($outFileAction)<=0 || ($outFileAction=="H")) echo "checked";?>><label for="outFileAction_H"><?echo GetMessage("CATI_OF_DEACT") ?></label><br>
			<input type="radio" id="outFileAction_D" name="outFileAction" value="D" <?if ($outFileAction=="D") echo "checked";?>><label for="outFileAction_D"><?echo GetMessage("CATI_OF_DEL") ?></label><br>
			<input type="radio" id="outFileAction_F" name="outFileAction" value="F" <?if ($outFileAction=="F") echo "checked";?>><label for="outFileAction_F"><?echo GetMessage("CATI_OF_KEEP") ?></label>
		</td>
	</tr>
	<tr>
		<td valign="top"><?echo GetMessage("CATI_INACTIVE_PRODS");?>:</td>
		<td valign="top">
			<input type="radio" id="inFileAction_F" name="inFileAction" value="F" <?if (strlen($inFileAction)<=0 || ($inFileAction=="F")) echo "checked";?>><label for="inFileAction_F"><?echo GetMessage("CATI_KEEP_AS_IS");?></label><br>
			<input type="radio" id="inFileAction_A" name="inFileAction" value="A" <?if ($inFileAction=="A") echo "checked";?>><label for="inFileAction_A"><?echo GetMessage("CATI_ACTIVATE_PROD");?></label>
		</td>
	</tr>
	<tr>
		<td valign="top"><?echo GetMessage("CATI_AUTO_STEP_TIME");?>:</td>
...
<?if ($STEP < 4):?>
	...
	<?endif?>
	<input type="submit" value="<?echo ($STEP==3)?GetMessage("CATI_NEXT_STEP_F"):GetMessage("CATI_NEXT_STEP") ?> &gt;&gt;" name="submit_btn">
	...
	?>
<?else:?>
	<input type="submit" name="backButton2" value="&lt;&lt; <?echo GetMessage("ICI_2_1_STEP") ?>">
<?endif;?>
