...
if($_REQUEST["Import"]=="Y" && (array_key_exists("NS", $_REQUEST) || array_key_exists("outFileAction", $_REQUEST)))
{
	...
	if(array_key_exists("NS", $_REQUEST))
		...
		$NS = array(
			...
			"PREVIEW" => $_REQUEST["GENERATE_PREVIEW"]=="Y"? array(intval($_REQUEST["PREVIEW_WIDTH"]), intval($_REQUEST["PREVIEW_HEIGHT"])): false,
		);
	...
	//Calculate file name to be loaded and directory name for "pictures"
	...
	if(count($arErrors)==0):?>
		...
		<?else:?>
			...
			<?echo GetMessage("IBLOCK_CML2_WITH_ERRORS", array("#COUNT#"=>intval($NS["DONE"]["ERR"])))?></p>
		<?endif;?>
	<?endif;
...
{
	var queryString='Import=Y&lang=<?=LANG?>';
	if(bFirst)
	{
		...
		queryString+='&INTERVAL='+document.getElementById('INTERVAL').value;
		...
		if(document.getElementById('GENERATE_PREVIEW').checked)
		{
			...
			queryString+='&PREVIEW_WIDTH='+document.getElementById('PREVIEW_WIDTH').value;
			queryString+='&PREVIEW_HEIGHT='+document.getElementById('PREVIEW_HEIGHT').value;
		}
	}
	else
	{
	...
	ShowWaitWindow();
	CHttpRequest.Send('iblock_xml_import.php?'+queryString);
}
...
?>
	...
	<tr valign="top">
		<td><?echo GetMessage("IBLOCK_CML2_GENERATE_PREVIEW")?>:</td>
		<td>
			<input type="checkbox" id="GENERATE_PREVIEW" name="GENERATE_PREVIEW" value="Y" checked>
		</td>
	</tr>
	<tr valign="top">
		<td><?echo GetMessage("IBLOCK_CML2_PREVIEW_WIDTH")?>:</td>
		<td>
			<input type="text" id="PREVIEW_WIDTH" name="PREVIEW_WIDTH" size="5" value="100">
		</td>
	</tr>
	<tr valign="top">
		<td><?echo GetMessage("IBLOCK_CML2_PREVIEW_HEIGHT")?>:</td>
		<td>
			<input type="text" id="PREVIEW_HEIGHT" name="PREVIEW_HEIGHT" size="5" value="100">
		</td>
	</tr>
<?$tabControl->Buttons();?>
