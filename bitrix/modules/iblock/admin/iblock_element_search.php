...
$sTableID = "tbl_iblock_el_search_".intval($arIBlock["ID"]);
...
$arHeader[] = array("id"=>"ACTIVE", "content"=>GetMessage("IBLOCK_ELEMENT_ADMIN_ACTIVE"), "sort"=>"active", "align"=>"center", "default"=>true);
...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	$row->AddViewField("NAME", $f_NAME."<input type=hidden name='n".$f_ID."' id='name_".$f_ID."' value='".addslashes(htmlspecialchars($f_NAME))."'>");
	$row->AddViewField("USER_NAME", "[<a target=\"_blank\" href=\"user_edit.php?lang=".LANGUAGE_ID."&ID=$f_MODIFIED_BY\">$f_MODIFIED_BY</a>]&nbsp;$f_USER_NAME");
	$row->AddCheckField("ACTIVE");
	$row->AddViewField("CREATED_USER_NAME", "[<a target=\"_blank\" href=\"user_edit.php?lang=".LANGUAGE_ID."&ID=$f_CREATED_BY\">$f_CREATED_BY</a>]&nbsp;$f_CREATED_USER_NAME");
	$row->AddViewField("PREVIEW_PICTURE", CFile::ShowFile($f_PREVIEW_PICTURE, 100000, 50, 50, true));
	$row->AddViewField("DETAIL_PICTURE", CFile::ShowFile($f_DETAIL_PICTURE, 100000, 50, 50, true));

	$row->AddViewField("WF_STATUS_ID", CIblockElement::WF_GetStatusTitle($f_WF_STATUS_ID)."<input type=hidden name='n$f_ID' value='".addslashes($f_NAME)."'>");
	$row->AddViewField("LOCKED_USER_NAME", '&nbsp;<a href="user_edit.php?lang='.LANG.'&ID='.$f_WF_LOCKED_BY.'" title="'.GetMessage("IBLOCK_ELSEARCH_USERINFO").'">'.$f_LOCKED_USER_NAME.'</a>');
	...
	foreach($arSelectedProps as $aProp)
	{
		...
		else
		{
			...
			$dbPVals = CIBlockElement::GetProperty($IBLOCK_ID, $f_ID, $xxord, $xxby, Array("ID"=>$aProp['ID']));
	...
	$row->AddActions(array(
		array(
			...
			"ACTION"=>"javascript:SelEl('".addslashes($get_xml_id=="Y"?$f_XML_ID:$f_ID)."', '".addslashes($f_NAME)."')",
		),
	));
}
...
$arFindFields = Array(
		"type"=>GetMessage("IBLOCK_ELSEARCH_F_TYPE"),
		"id"=>"ID",
...
$oFilter = new CAdminFilter("tbl_iblock_el_search_filter", $arFindFields);
...
function SelAll()
{
	var frm = document.getElementById('form_tbl_iblock_el_search_<?=intval($arIBlock["ID"])?>');
	if(frm)
	{
...
//-->
	...
	<tr>
		<td><?echo GetMessage("IBLOCK_ELSEARCH_TYPE")?></td>
		<td>
			<nobr>
			<select name="filter_type">
			<option value=""><?=GetMessage("IBLOCK_VALUE_ANY")?></option>
			<?
			$db_types = CIBlockType::GetList();
			while($db_types->ExtractFields("str_type_")):
				$ibtypelang = CIBlockType::GetByIDLang($str_type_ID, LANGUAGE_ID);
			?>
			<option value="<?=$str_type_ID?>"<?if($IBLOCK_TYPE==$str_type_ID)echo " selected"?>><?=htmlspecialcharsex($ibtypelang["NAME"])?></option>
			<?endwhile?>
			</select>
			</nobr>
		</td>
	...
	<tr>
		<td  nowrap><? echo GetMessage("IBLOCK_F_TIMESTAMP")." (".CLang::GetDateFormat("SHORT")."):"?></td>
	...
	<tr>
		<td nowrap><?=GetMessage("IBLOCK_F_MODIFIED_BY")?></td>
	...
	<?if(CModule::IncludeModule("workflow")):?>
	<tr>
		<td nowrap><?=GetMessage("IBLOCK_F_STATUS")?></td>
	...
	<?if(is_array($arIBTYPE) && ($arIBTYPE["SECTIONS"] == "Y")):?>
	<tr>
		<td nowrap><?echo GetMessage("IBLOCK_F_SECTION")?></td>
		<td nowrap>
			<select name="filter_section">
				<option value=""><?echo GetMessage("IBLOCK_VALUE_ANY")?></option>
				<option value="0"<?if($filter_section=="0")echo" selected"?>><?echo GetMessage("IBLOCK_CONTENT")?></option>
				<?
			...
			<input type="checkbox" name="filter_subsections" value="Y"<?if($filter_subsections=="Y")echo" checked"?>> <?echo GetMessage("IBLOCK_INCLUDING_SUBSECTIONS")?>
	...
	<tr>
		<td nowrap><?echo GetMessage("IBLOCK_F_ACTIVE")?></td>
		<td nowrap>
	...
	<tr>
		<td nowrap><?echo GetMessage("IBLOCK_F_NAME")?></td>
		<td nowrap>
...
$oFilter->Buttons();
?>
<input type="hidden" name="get_xml_id" value="<?=$get_xml_id=="Y"?"Y":"N"?>">
...
<input type="submit" name="set_filter" value="<?=GetMessage("admin_lib_filter_set_butt")?>" title="<?=GetMessage("admin_lib_filter_set_butt_title")?>">
<input type="submit" name="del_filter" value="<?=GetMessage("admin_lib_filter_clear_butt")?>" title="<?=GetMessage("admin_lib_filter_clear_butt_title")?>">
<?$oFilter->End();?>
</form>
