...
$arResult["FORM_ACTION"] = isset($_SERVER["REDIRECT_URL"])? htmlspecialchars($_SERVER["REDIRECT_URL"]): "";
...
$CACHE_ID = serialize($arParams).serialize($USER->GetGroups());
if($obCache->StartDataCache($arParams["CACHE_TIME"], $CACHE_ID, $componentPath))
{
	...
	if(CModule::IncludeModule("catalog"))
	{
		...
		{
			if(in_array($arPrice["NAME"],$arParams["PRICE_CODE"]))
...
foreach($arParams["FIELD_CODE"] as $field_code)
{
	...
	if($field_res)
		$arResult["ITEMS"][] = array("NAME" => htmlspecialchars(GetMessage("IBLOCK_FIELD_".$field_code)), "INPUT" => $field_res);
}
...
{
	if(!array_key_exists($key, $arResult["arrInputNames"]))
	{
		$arResult["ITEMS"][] = array(
			"HIDDEN" => true,
