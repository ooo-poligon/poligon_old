...
function Get_arSnGroups(template)
{
	...
	_r.Send('/bitrix/admin/fileman_manage_snippets.php?templateID='+template+'&target=getgroups');
}
...
function checkFileName(callback, fileName, templateId, new_group)
{
	...
	try
	{
		cfn_CHttpRequest.Send('/bitrix/admin/fileman_manage_snippets.php?lang='+BXLang+'&site='+BXSite+'&templateID='+templateId+'&target=check&filename='+escape(fileName));
	}
	catch(e)
	{
		callback.apply(obj,[!snippet_file_exists,fileName,templateId]);
...
function saveSnippet(fileName, templateId, new_group)
{
	...
	postData += "title="+escape(title)+"&code="+escape(code)+"&filename="+escape(fileName)+"&description="+escape(description)+"&location="+escape(location)+"&new_group="+escape(new_group)+"&thumb="+escape(thumb);
	var _ss = new JCHttpRequest();
	_ss.Action = function(result){};
	try
	{
		_ss.Post('/bitrix/admin/fileman_manage_snippets.php?site='+BXSite+'&templateID='+templateId+'&target=add',postData);
	}
...
function editSnippet(title, code)
{
	...
	var pSessid = document.getElementById("sessid");
	var post_data = "sessid=" + pSessid.value + '&';
	...
	if (thumb != oEl.thumb)
	{
		...
		if (thumb != '')
			oEl.thumb = oEl.name.substr(0,oEl.name.lastIndexOf('.'))+thumb.substr(thumb.lastIndexOf('.'));
		else
			oEl.thumb = '';
	}
	...
	try
	{
		_es.Post('/bitrix/admin/fileman_manage_snippets.php?site='+BXSite+'&target=edit',post_data);
	}
...
<table class="add_snippet" border="0" cellpadding="2" style="height:100%; width:99%">
	...
	<tr>
		<td align="right" valign="top"><?=GetMessage("FILEMAN_ED_FILE_LOCATION")?>:</td>
		<td valign="top">
