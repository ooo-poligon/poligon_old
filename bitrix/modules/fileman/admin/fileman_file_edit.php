...
if (!$ismenu)
{
	$aDDMenuEdit[] = array(
		...
		"ACTION" => ($mode == 'text' ? "return; " : "" )."window.location='fileman_file_edit.php?".$addUrl."&amp;site=".Urlencode($site)."&amp;path=".UrlEncode($path).(strlen($new)>0? "&amp;new=Y":"").(strlen($back_url)>0? "&amp;back_url=".urlencode($back_url):"").(strlen($template)>0? "&amp;template=".urlencode($template):"")."';",
...
if($USER->CanDoOperation('edit_php'))
{
	$aDDMenuEdit[] = array(
		...
		"ACTION" => ($mode == 'php' ? "return; " : "" )."window.location='fileman_file_edit.php?".$addUrl."&amp;site=".Urlencode($site)."&amp;path=".UrlEncode($path)."&amp;full_src=Y".(strlen($new)>0? "&amp;new=Y":"").(strlen($back_url)>0? "&amp;back_url=".urlencode($back_url):"").(strlen($template)>0? "&amp;template=".urlencode($template):"")."';",
...
else
{
	$aDDMenuEdit[] = array(
		...
		"ACTION" => "window.location='"."fileman_html_edit.php?".$addUrl."&amp;site=".Urlencode($site)."&amp;path=".UrlEncode($path).(strlen($new)>0? "&amp;new=Y":"").(strlen($back_url)>0? "&amp;back_url=".urlencode($back_url):"").(strlen($template)>0? "&amp;template=".urlencode($template):"")."';"
	);
}
