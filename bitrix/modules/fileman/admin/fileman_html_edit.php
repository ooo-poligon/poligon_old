...
{
	include($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/fileman/admin/fileman_html_edit_1.php");
	die();
}
...
function finalLPAreplacer($str)
{
	...
	$str = preg_replace("/(<script[^>]*language\s*=\s*['|\"]php['|\"][^>]*>)([\s|\S]*?)(<\/script>)/i", "&lt;?\\2?&gt;", $str);
	return $str;
}
...
{
	$str = preg_replace("/[^a-zA-Z0-9_:\.]/is", "", $str);
	return $str;
}
...
//if((strlen($new) <= 0 || strtolower($new) != 'y' || strlen($filename) <= 0) && !is_file($abs_path))
...
else
{
	...
	{
		if(!is_file($abs_path))
			$strWarning = GetMessage("FILEMAN_FILEEDIT_FOLLDER_EXISTS")." ";
		else
			$bEdit = true;
	}
...
/*if($bFullScreen && strlen($strWarning)<=0){?><script>try{document.body.style.display = 'none';}catch(e){};</script><?} */
?>
...
if (!$ismenu)
{
	$aDDMenuEdit[] = array(
		...
		"ACTION" => "window.location='fileman_file_edit.php?".$addUrl."&amp;site=".Urlencode($site)."&amp;path=".UrlEncode($path).(strlen($new)>0? "&amp;new=Y":"").(strlen($back_url)>0? "&amp;back_url=".urlencode($back_url):"").(strlen($template)>0? "&amp;template=".urlencode($template):"")."';",
	);
}
...
if($USER->CanDoOperation('edit_php'))
{
	$aDDMenuEdit[] = array(
		...
		"ACTION" => "window.location='fileman_file_edit.php?".$addUrl."&amp;site=".Urlencode($site)."&amp;path=".UrlEncode($path)."&amp;full_src=Y".(strlen($new)>0? "&amp;new=Y":"").(strlen($back_url)>0? "&amp;back_url=".urlencode($back_url):"").(strlen($template)>0? "&amp;template=".urlencode($template):"")."';",
	);
}
...
{
	window.location='/bitrix/admin/fileman_html_edit.php?lang=<?echo LANG?>&site=<?=Urlencode($site)?>&path=<?echo UrlEncode($path)?>&new=y&template='+id;
...
?>
	...
	<?if(!$bEdit):?>
		...
		<?$arTemplates = CFileman::GetFileTemplates(LANGUAGE_ID, array($site_template));?>
		<tr><td><?echo GetMessage("FILEMAN_FILEEDIT_TEMPLATE")?></td>
		<td>
		<script>
		function templateOnChange(_this)
		{
			...
			window.location='/bitrix/admin/fileman_html_edit.php?lang=<?echo LANG?>&site=<?=Urlencode($site)?>&path=<?echo UrlEncode($path)?>&new=y&template='+escape(_this[_this.selectedIndex].value)+_name+_title;
		}
		</script>
		<?
		...
		<tr valign="top"><td><?echo GetMessage("FILEMAN_FILEEDIT_NAME")?></td>
		<td>
			<?if (isset($filename2))
				$filename = $filename2;?>
			<input type="text" name="filename" id="filename" style="float: left;" size="30" maxlength="255" value="<?echo htmlspecialchars($filename)?>">
	...
	<tr>
		...
		<td width="70%"><input type="text" id="title" name="title" size="60" maxlength="255" value="<?echo htmlspecialchars($title)?>"></td>
	</tr>
	<tr>
	<td colspan="2">
		<script>
	...
	<tr>
		<td>
			<table border="0" cellspacing="1" cellpadding="3" id="proptab"  class="internal">
				<tr class="heading">
					<td><?echo GetMessage("FILEMAN_H_EDIT_PROP")?></td>
					<td><?echo GetMessage("FILEMAN_EDIT_PROPSVAL")?></td>
				</tr>
				<?
	...
	?>
	<tr>
		<td width="40%"><label for="add_to_menu"><?echo GetMessage("FILEMAN_H_EDIT_ADD")?></label></td>
	...
	<tr id="ex"<?if($_POST['add_to_menu']!='Y') echo ' style="display:none;"';?>>
		<td><?echo GetMessage("FILEMAN_H_EDIT_TMENU")?></td>
		<td>
			...
			<?
				...
				foreach ($armt as $key => $title)
				{
					...
					if($strSelected=="")
						$strSelected = $key;
					?><option value="<?echo htmlspecialchars($key)?>"
					<?if(isset($_POST['menutype']) && $_POST['menutype'] == $key) echo 'selected';?>>
					<?echo htmlspecialchars($title." [".$key."]")?></option><?
				}
			?>
			</select>
		</td>
...
<tr id="e0"<?if($_REQUEST['add_to_menu']!='Y')echo ' style="display:none;"';?>>
	<td valign="top"><?echo GetMessage("FILEMAN_H_EDIT_MENUIT")?></td>
	<td>
		<input type="radio" name="itemtype" id="itemtype_n" value="n" onclick="chitemtype()"
		<?if($n = (!isset($_POST['itemtype']) || $_POST['itemtype'] != 'e')) echo 'checked';?>> <label for="itemtype_n"><?echo GetMessage("FILEMAN_H_EDIT_MENUITNEW")?></label><br>
...
<script>
<?if($_REQUEST['add_to_menu']=='Y' && $add_to_menu_check)echo ' chtyp();';?>
</script>
<br>
<br>
<br>
