...
function OnSave()
{
	...
	if(pElement && pElement.getAttribute && pElement.getAttribute("__bxtagname")=="anchor")
	{
		if(anchor_value.value.length<=0)
			...
			pElement.setAttribute("__bxcontainer", BXSerialize({"name":anchor_value.value}));
	}
	else
	{
		if(anchor_value.value.length>0)
		{
			...
			pObj.pMainObj.insertHTML('<img id="'+tmp_id+'" src="/bitrix/images/1.gif" style="background-image: url(/bitrix/images/fileman/htmledit2/_global_iconkit.gif); background-position: -260px 0; height: 20px; width: 20px"  __bxtagname="anchor" __bxcontainer="'+bxhtmlspecialchars(BXSerialize({name : anchor_value.value}))+'" />');
...
function _LPreview()
{
	...
	if (!w) w = parseInt(this.offsetWidth);

	var h = this.getAttribute('height');
	if (!h) h = parseInt(this.offsetHeight);
		...
		<td width="50%">
			<table cellpadding="0" cellspacing="0">
				<tr>
					...
					<td><input type="button" value="..." id="OpenFileBrowserWindImage_button" onclick=""></td>
				</tr>
			</table>
		</td>
	</tr>
...
function _displayTaskbarList(oCont)
{
	...
	if(pObj.pMainObj.allowedTaskbars['BXSnippetsTaskbar'])
	{
		...
		//}
		//catch(e){_alert('Error show snippet row');}
	}
...
function _displayAdditionalProps(oCont)
{
	...
	oTd.width = "70%";
	oTd = oTr.insertCell(-1);
	var pCheckbox = oTd.appendChild(pObj.pMainObj.CreateElement("INPUT", {type: 'checkbox', id: '__bx_show_tooltips'}));
	...
	oTr.className = "bxpropertysell";
	oTd = oTr.insertCell(-1);
	oTd.innerHTML = '<?=GetMessage("FILEMAN_ED_VIS_EFFECTS")?>';
	oTd.align = "right";
	oTd = oTr.insertCell(-1);
	var pCheckbox = oTd.appendChild(pObj.pMainObj.CreateElement("INPUT", {type: 'checkbox', id: '__bx_visual_effects'}));
