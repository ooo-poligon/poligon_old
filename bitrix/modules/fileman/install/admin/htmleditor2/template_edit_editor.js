...
var insert_wa = [
	'BXButton', 
	{
		...
		handler : function ()
		{
			this.pMainObj.insertHTML('<IMG src="/bitrix/images/fileman/htmledit2/work_area.gif"  __bxtagname="work_area"/>');
		}
	}
];
...
function ReplaceSiteTemplatePath(str)
{
	...
	str = str.replace(/(<img(?:\s|\S)*?src\s*=\s*)("|')((?:(?:\s|\S)*?[^\\]{1}){1})\2((?:(?:\s|\S)*?[^\?]{1})??>)/ig, replace);
	str = str.replace(/(<img(?:\s|\S)*?__bxsrc\s*=\s*)("|')((?:(?:\s|\S)*?[^\\]{1}){1})\2((?:(?:\s|\S)*?[^\?]{1})??>)/ig, replace);
	return str;
}
