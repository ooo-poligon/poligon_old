...
{
	$arFields = array("SORT" => intVal(intVal($SORT) <= 0 ? 150 : $SORT));
	for ($i = 0; $i<count($arSysLangs); $i++)
	{
		$arFields["LANG"][] = array(
			"LID" => $arSysLangs[$i],
			"NAME" => $_REQUEST["FORUM_GROUP"]["LANG"][$arSysLangs[$i]]["NAME"],
			"DESCRIPTION" => $_REQUEST["FORUM_GROUP"]["LID"][$arSysLangs[$i]]["DESCRIPTION"]);
	}
	if (!CForumGroup::CheckFields(($ID > 0 ? "UPDATE" : "ADD"), $arFields))
	{
		$arError[] = array(
...
if ($ID>0)
{
	...
	$db_group->ExtractFields("str_", True);
}
