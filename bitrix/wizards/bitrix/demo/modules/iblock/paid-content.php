...
//Import XML
if($IBLOCK_ID = DEMO_IBlock_ImportXML("110_paid_paid-articles_ru.xml", $arParams["site_id"], false))
{
	$_SESSION["DEMO_IBLOCK_PAID"] = CIBlockCMLImport::GetIBlockByXML_ID("paid-subscribe");
	if($SUBSCR_ID = DEMO_IBlock_ImportXML("120_paid_paid-subscribe_ru.xml", $arParams["site_id"], false))
	{
		...
		CopyDirFiles(dirname(__FILE__)."/public/e-store/paid/images/", $_SERVER["DOCUMENT_ROOT"]."/e-store/paid/images", true, true);
		...
		if($_SESSION["DEMO_IBLOCK_PAID"] === false)
			DEMO_IBlock_ImportXML("120_paid_paid-subscribe-offers_ru.xml", $arParams["site_id"], true);
	}
}
?>