...
class CustomizeSiteTheme extends CWizardStep
{
	...
	function OnPostForm()
	{
		...
		$themePath = $_SERVER["DOCUMENT_ROOT"].$wizard->GetPath()."/templates/".$templateID."/themes";
	...
	function ShowStep()
	{
		...
		$themePath = $wizard->GetPath()."/templates/".$templateID."/themes";
...
class SelectServices extends CWizardStep
{
	function InitStep()
	{
		$this->SetTitle("Выбор сервисов (шаг 4 из 4)");
		$this->SetSubTitle("Выбранные сервисы будут установлены на сайте");
