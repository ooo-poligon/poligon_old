...
function add2b()
{
	if (document.getElementById('min_quantity'))
	{
		if (parseInt(document.getElementById('min_quantity').value) > parseInt(document.getElementById('qua').value))
			alert('Минимальное кол-во для покупки: '+document.getElementById('min_quantity').value+'шт.');
		else {
...
<table cellpadding=5 cellspacing=0 id="_table" width="380">
	...
	<tr>
		<td colspan="3"><b>
			...
			<?
				if ($min_quan)
					echo 'Минимальное кол-во для покупки: '.$min_quan.' шт <input type="hidden" id="min_quantity" value='.$min_quan.'>'?>

		</td>
	</tr>
	<tr>
		<Td width=100>Количество*:</td><td width=70><input id="qua" name="quantity" type="text" size="4" value=""></td><td><input type="submit" onClick="add2b()" value="Добавить"><input type="button" value="X" onClick="self.close()"></td>
	</tr>
	<tr>
...
echo '* Данный товар можно купить только в количестве от '.$min_quan.' шт';
echo '</div>';
}
?>
</form>
